...
<?php
	...
	// IDの取得
	if($err_flg == 0){
		$sql = "SELECT id FROM event_list WHERE hiduke >= \"".$kyou."\" AND k_use = 1";
	...
	if($err_flg == 0){
		...
		for($i=0; $i<count($id); $i++){
			$sql  = "SELECT email FROM entrylist WHERE seminarid = \"".$id[$i]."\" ";
			$sql .= "AND (stat = 1 OR (stat = 0 AND wait = 1) )";
			$sql  = mb_convert_encoding($sql, "SJIS", "UTF-8");
	...
	if($err_flg == 0){
		...
		while($i<count($email)){
			...
			$sql .= "VALUES(NULL, '40', \"".$email[$i]."\", NULL, NULL, NULL, '2',\"". $date."\", 0, NULL)";
			$result = mysql_query($sql, $link);

			$sql2  = "INSERT INTO haihai(seq, mdptn, email, kyoten, namae, url, ptnid, insdate, sent, sentdate)";
			$sql2 .= "VALUES(NULL, '40', \"".$email[$i]."\", NULL, NULL, NULL, '3',\"". $date."\", 0, NULL)";
			$result2 = mysql_query($sql2, $link);

			$sql3  = "INSERT INTO haihai(seq, mdptn, email, kyoten, namae, url, ptnid, insdate, sent, sentdate)";
			$sql3 .= "VALUES(NULL, '40', \"".$email[$i]."\", NULL, NULL, NULL, '5',\"". $date."\", 0, NULL)";
			$result3 = mysql_query($sql3, $link);

			$sql4  = "INSERT INTO haihai(seq, mdptn, email, kyoten, namae, url, ptnid, insdate, sent, sentdate)";
			$sql4 .= "VALUES(NULL, '40', \"".$email[$i]."\", NULL, NULL, NULL, '7',\"". $date."\", 0, NULL)";
	...
	$title = "【ステップ登録】「未来セミナー予約者パターン削除」";
	$main	 = "開始日時：".$start_time."\r\n";
	$main	.= "終了日時：".$end_time."\r\n";
	$main	.= "処理件数：".($all_cnt*4)."件\r\n";
	$main	.= "エラー　：";
	if($err_flg == 0){
		$main .= "なし";
	}else{
