...
<?php
	...
	// クエリを送信する
	if($err_flg == 0){
		$sql = "SELECT vmail, vtype, vname1, vcheck FROM maillist WHERE vsend = '1' AND haihai = 1";
	...
	if($err_flg == 0){
		...
		while ($row = mysql_fetch_assoc($result)) {
			...
			$kyoten[$i]  = $row['vtype'];
			$namae[$i] 	 = $row['vname1'];
			$set_url[$i] = 'http://www.jawhm.or.jp/mail/?u='.$row['vcheck'].'&m='.md5($email[$i]);
		...
		for($i=0; $i<count($email); $i++){
			...
			$sql .= "VALUES(NULL, '10', \"".$email[$i]."\",\"". $kyoten[$i]."\",\"". $namae[$i]."\",";
			$sql .= "\"".$set_url[$i]."\",NULL,\"". $date."\", 0, NULL)";
			$result = mysql_query($sql, $link);
