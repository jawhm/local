...
<div id="maincontent">
    ...
    <h2 class="sec-title">配信停止フォーム</h2>
    <div id="sitemapbox">
...
//                    $sql = 'UPDATE maillist SET haihai = 1, vname1 = :vname1 , vsend = :vsend , vstat = :vstat , udate = :udate WHERE vcheck = :vcheck';
                    $sql = 'UPDATE maillist SET haihai = 1, vname1 = :vname1 , riyu = :riyu , vstat = :vstat , udate = :udate WHERE vcheck = :vcheck';
                    $stt2 = $db->prepare($sql);
...
//                    $sql = 'UPDATE maillist SET haihai = 1, vsend = :vsend , vstat = :vstat , udate = :udate WHERE vmail = :vmail';
                    $sql = 'UPDATE maillist SET haihai = 1 , vstat = :vstat , udate = :udate WHERE vmail = :vmail';
                    $stt2 = $db->prepare($sql);
//                    $stt2->bindValue(':vsend', $vsend);
                    $stt2->bindValue(':vstat', '登録');
                    $stt2->bindValue(':udate', date('Y/m/d'));
                    ...
                    <table class="mailtable">
                        <tr>
                            ...
                            <td class="mailtable-data">
                                <input type="text" size="20" maxlength="20" value="<?php print $row['vmail']; ?>" readonly>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td class="mailtable-data">
                                <select id="riyu" name="riyu">
                                    ...
                                    <option value="メルマガの配信頻度が少ない">メルマガの配信頻度が少ない</option>
                                    <option value="メルマガの内容が合わない">メルマガの内容が合わない</option>
                                    <option value="メールの内容量が多い">メールの内容量が多い</option>
                                    <option value="メールの内容量が少ない">メールの内容量が少ない</option>
                                    <option value="欲しいプレゼントや参加したいモニター情報が無い">欲しいプレゼントや参加したいモニター情報が無い</option>
                                    <option value="欲しいプレゼントや参加したいモニターに応募しても当たらない">欲しいプレゼントや参加したいモニターに応募しても当たらない</option>
                                    <option value="コインがなかなか貯まらない">コインがなかなか貯まらない</option>
                                    <option value="サービスが分かりにくい">サービスが分かりにくい</option>
                                    <option value="サイトのデザインが悪い・使いにくい">サイトのデザインが悪い・使いにくい</option>
                                    <option value="サイトを訪問する時間がない">サイトを訪問する時間がない</option>
                                    <option value="登録した覚えがない">登録した覚えがない</option>
                                    <option value="ニッセンを利用しなくなった">ニッセンを利用しなくなった</option>
...
//                                }
                ...
                <p>また、システムの都合上、配信状態を「配信停止」に変更した後、４８時間程度はメールが送信されてしまう可能性があります。恐れ入りますがご了承ください。</p>
                <p></p>
                <p></p>
                <?php
            }
