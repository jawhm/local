<?php
	...
	function count_number_of_seminar($year,$month,$keyword,$navigation_used=0)
	{
		...
		$rs = $db->query('SELECT count(*) as number FROM event_list WHERE  hiduke <= \''.$limit_date['ending_date_yy'].'-'.$new_format_ending_month.'-'.$new_format_ending_day.'\' and hiduke >= \''.$limit_date['beginning_date_yy'].'-'.$new_format_beginning_month.'-'.$new_format_beginning_day.'\' and ( k_use = 1 or preuse = 1 ) '.$keyword.' ORDER BY hiduke, starttime, id');
	...
	function get_days_list ($place_name,$keyword,$letter_type='A')
	{
		...
		try {
			...
			//SQL QUERY
			$rs = $db->query('SELECT id, hiduke, year(hiduke) as yy, month(hiduke) as mm, day(hiduke) as dd, date_format(starttime, \'%c月%e日 (%a) %k:%i\') as start, date_format(starttime, \'%k:%i\') as starttime, title, memo, place, k_use, k_title1, k_desc1, k_stat, free, pax, booking, group_color, broadcasting, indicated_option, country_code, short_description, seminar_name FROM event_list WHERE ( k_use = 1 or preuse = 1 ) '.$keyword.' ORDER BY hiduke, starttime, id');
