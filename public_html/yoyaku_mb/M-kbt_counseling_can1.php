...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'M-kbt_counseling_can1.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('M-kbt_counseling_kkn.php');
...
//print('<input type="submit" name="button" style="padding: 10px 10px 10px 10px;" value="予約する">');
			...
			//*** スマホ ************************************************************************************
				...
				print('<font color="blue">以下の予約を取り消します。</font><br><font size="2" color="red">（まだ予約取消は行われていません。）</font>');
...
//				print('</tr>');
				...
				print('<table border="0">');
				print('<tr>');
				print('<td width="210" align="right" valign="middle">');
				//「予約を取消する場合はこのボタンを押してください。」
				//print('<img src="./img_' . $lang_cd . '/title_kbt_can_kknbtn.png" border="0">');
				print('<font size="2" color="red">このボタンで予約取消します -></font>');
				print('</td>');
				//予約の取消しボタン
				print('<td width="100" align="center" valign="middle">');
				if( $mobile_kbn == 'I' ){
					//iPhoneのみ
					print('<form method="post" action="' . $sv_https_adr . 'yoyaku_mb/M-kbt_counseling_can2.php#kbtcounseling">');
				}else{
					print('<form method="post" action="' . $sv_https_adr . 'yoyaku_mb/M-kbt_counseling_can2.php">');
				}
				print('<input type="hidden" name="prc_gmn" value="' . $gmn_id . '">');
