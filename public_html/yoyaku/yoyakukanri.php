<?php
	...
	if (date('H') == 20)	{
		...
		try {
			...
			$db2->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$db2->query('SET CHARACTER SET utf8');
			$stt2 = $db2->prepare('SELECT id, place, hiduke, year(hiduke) as y, month(hiduke) as m, day(hiduke) as d, date_format(hiduke,\'%w\') as yobi, date_format(starttime,\'%k:%i\') as sttime, k_title1, pax, booking, waitting, new, free FROM event_list WHERE place = \'tokyo\' and starttime <= date_add(now(), interval "0 0" day_hour ) and hiduke = date_sub(current_date(), INTERVAL 0 DAY) ');
			$stt2->execute();
			while($row2 = $stt2->fetch(PDO::FETCH_ASSOC)){
				// イベント情報読み込み
				$cur_id = $row2['id'];
				...
				$db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
				$db->query('SET CHARACTER SET utf8');
				$stt = $db->prepare('SELECT * FROM mailtext WHERE keycd = "auto_seminar_shusseki_tokyo"');
				$stt->execute();
				while($row = $stt->fetch(PDO::FETCH_ASSOC)){
					$cur_text1 = $row['text1'];
					$cur_text2 = $row['text2'];
				...
				$stt->execute();
				$idx = 0;
        echo '<hr><pre>'; var_dump($stt);echo '<br>';
	msgout ('----- here -----');exit;
				while($row = $stt->fetch(PDO::FETCH_ASSOC)){
					// 予約情報読み込み
					$idx++;
		...
		try {
			...
			$db2->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$db2->query('SET CHARACTER SET utf8');
			$stt2 = $db2->prepare('SELECT id, place, hiduke, year(hiduke) as y, month(hiduke) as m, day(hiduke) as d, date_format(hiduke,\'%w\') as yobi, date_format(starttime,\'%k:%i\') as sttime, k_title1, pax, booking, waitting, new, free FROM event_list WHERE place = \'nagoya\' and starttime <= date_add(now(), interval "0 0" day_hour ) and hiduke = date_sub(current_date(), INTERVAL 0 DAY) ');
			$stt2->execute();
			while($row2 = $stt2->fetch(PDO::FETCH_ASSOC)){
				// イベント情報読み込み
				$cur_id = $row2['id'];
				...
				$db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
				$db->query('SET CHARACTER SET utf8');
				$stt = $db->prepare('SELECT * FROM mailtext WHERE keycd = "auto_seminar_shusseki_nagoya"');
				$stt->execute();
				while($row = $stt->fetch(PDO::FETCH_ASSOC)){
					$cur_text1 = $row['text1'];
					$cur_text2 = $row['text2'];
				...
				while($row = $stt->fetch(PDO::FETCH_ASSOC)){
					...
					if ($cur_text2 <> '' && $cur_new <> 1 && $cur_free <> 1)	{
						...
						$body .= chr(10);
						$body .= '';
						$from = mb_encode_mimeheader(mb_convert_encoding("日本ワーキングホリデー協会","JIS"))."<sodan-nagoya@jawhm.or.jp>";
						mb_send_mail($cur_email,$subject,$body,"From:".$from);
		...
		try {
			...
			$db2->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$db2->query('SET CHARACTER SET utf8');
			$stt2 = $db2->prepare('SELECT id, place, hiduke, year(hiduke) as y, month(hiduke) as m, day(hiduke) as d, date_format(hiduke,\'%w\') as yobi, date_format(starttime,\'%k:%i\') as sttime, k_title1, pax, booking, waitting, new, free FROM event_list WHERE place = \'osaka\' and starttime <= date_add(now(), interval "0 0" day_hour ) and hiduke = date_sub(current_date(), INTERVAL 0 DAY) ');
			$stt2->execute();
			while($row2 = $stt2->fetch(PDO::FETCH_ASSOC)){
				// イベント情報読み込み
				$cur_id = $row2['id'];
				...
				$db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
				$db->query('SET CHARACTER SET utf8');
				$stt = $db->prepare('SELECT * FROM mailtext WHERE keycd = "auto_seminar_shusseki_osaka"');
				$stt->execute();
				while($row = $stt->fetch(PDO::FETCH_ASSOC)){
					$cur_text1 = $row['text1'];
					$cur_text2 = $row['text2'];
				...
				while($row = $stt->fetch(PDO::FETCH_ASSOC)){
					...
					if ($cur_text2 <> '' && $cur_new <> 1 && $cur_free <> 1)	{
						...
						$body .= chr(10);
						$body .= '';
						$from = mb_encode_mimeheader(mb_convert_encoding("日本ワーキングホリデー協会","JIS"))."<sodan-osaka@jawhm.or.jp>";
					//	mb_send_mail($cur_email,$subject,$body,"From:".$from);
		...
		try {
			...
			$db2->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$db2->query('SET CHARACTER SET utf8');
			$stt2 = $db2->prepare('SELECT id, place, hiduke, year(hiduke) as y, month(hiduke) as m, day(hiduke) as d, date_format(hiduke,\'%w\') as yobi, date_format(starttime,\'%k:%i\') as sttime, k_title1, pax, booking, waitting, new, free FROM event_list WHERE ( k_title1 LIKE \'%福岡%\' or place = \'fukuoka\' ) and starttime <= date_add(now(), interval "0 0" day_hour ) and hiduke = date_sub(current_date(), INTERVAL 0 DAY) ');
			$stt2->execute();
			while($row2 = $stt2->fetch(PDO::FETCH_ASSOC)){
				// イベント情報読み込み
				$cur_id = $row2['id'];
				...
				$db->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
				$db->query('SET CHARACTER SET utf8');
				$stt = $db->prepare('SELECT * FROM mailtext WHERE keycd = "auto_seminar_shusseki_fukuoka"');
				$stt->execute();
				while($row = $stt->fetch(PDO::FETCH_ASSOC)){
					$cur_text1 = $row['text1'];
					$cur_text2 = $row['text2'];
				...
				while($row = $stt->fetch(PDO::FETCH_ASSOC)){
					...
					if ($cur_text2 <> '' && $cur_new <> 1 && $cur_free <> 1)	{
						...
						$body .= chr(10);
						$body .= '';
						$from = mb_encode_mimeheader(mb_convert_encoding("日本ワーキングホリデー協会","JIS"))."<sodan-fukuoka@jawhm.or.jp>";
						mb_send_mail($cur_email,$subject,$body,"From:".$from);
	...
	if (date('H') == 12)	{
		...
		try {
			...
			$db2->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$db2->query('SET CHARACTER SET utf8');
			$stt2 = $db2->prepare('SELECT id, place, hiduke, year(hiduke) as y, month(hiduke) as m, day(hiduke) as d, date_format(hiduke,\'%w\') as yobi, date_format(starttime,\'%k:%i\') as sttime, k_title1, pax, booking, waitting, new, free FROM event_list WHERE place = \'tokyo\' and hiduke = date_sub(current_date(), INTERVAL 1 DAY) ');
			$stt2->execute();
			while($row2 = $stt2->fetch(PDO::FETCH_ASSOC)){
				// イベント情報読み込み
				$cur_id = $row2['id'];
		...
		try {
			...
			$db2->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$db2->query('SET CHARACTER SET utf8');
			$stt2 = $db2->prepare('SELECT id, place, hiduke, year(hiduke) as y, month(hiduke) as m, day(hiduke) as d, date_format(hiduke,\'%w\') as yobi, date_format(starttime,\'%k:%i\') as sttime, k_title1, pax, booking, waitting, new, free FROM event_list WHERE place = \'nagoya\' and hiduke = date_sub(current_date(), INTERVAL 1 DAY) ');
			$stt2->execute();
			while($row2 = $stt2->fetch(PDO::FETCH_ASSOC)){
				// イベント情報読み込み
				$cur_id = $row2['id'];
		...
		try {
			...
			$db2->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$db2->query('SET CHARACTER SET utf8');
			$stt2 = $db2->prepare('SELECT id, place, hiduke, year(hiduke) as y, month(hiduke) as m, day(hiduke) as d, date_format(hiduke,\'%w\') as yobi, date_format(starttime,\'%k:%i\') as sttime, k_title1, pax, booking, waitting, new, free FROM event_list WHERE place = \'osaka\' and hiduke = date_sub(current_date(), INTERVAL 1 DAY) ');
			$stt2->execute();
			while($row2 = $stt2->fetch(PDO::FETCH_ASSOC)){
				// イベント情報読み込み
				$cur_id = $row2['id'];
		...
		try {
			...
			$db2->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$db2->query('SET CHARACTER SET utf8');
			$stt2 = $db2->prepare('SELECT id, place, hiduke, year(hiduke) as y, month(hiduke) as m, day(hiduke) as d, date_format(hiduke,\'%w\') as yobi, date_format(starttime,\'%k:%i\') as sttime, k_title1, pax, booking, waitting, new, free FROM event_list WHERE ( k_title1 LIKE \'%福岡%\' or place = \'fukuoka\' ) and hiduke = date_sub(current_date(), INTERVAL 1 DAY) ');
			$stt2->execute();
			while($row2 = $stt2->fetch(PDO::FETCH_ASSOC)){
				// イベント情報読み込み
				$cur_id = $row2['id'];
