...
<?php
	...
	//引数の入力
	//（なし）
	print('個別カウンセリング　当日メール送信バッチ start[' . date( "Y-m-d H:i:s", time() ) . ']<br>');
	flush();
	ob_flush();
	...
	if( $svconnect_rtncd == 1 ){
		...
		$log_naiyou = 'サーバー接続に失敗しました。当日メール送信を中止しました。';	//内容
		...
		$log_office_cd = '';			//オフィスコード
		$log_kaiin_no = 'batch';		//会員番号 または スタッフコード
		$log_naiyou = '<font color=blue>個別カウンセリング 当日メール送信 を開始します。</font>';	//内容
		$log_err_inf = '';	//エラー情報
		require( './zy_log.php' );
		//************

		//処理当日の個別カウンセリングを予約している予約番号を取得する
		$yyk_cnt = 0;
		$query = 'select YYK_NO,KAIIN_ID from D_CLASS_YYK where KG_CD = "' . $DEF_kg_cd . '" and YMD = "' . $now_yyyymmdd . '" and TJT_MAIL_SEND_FLG = 0;';
...
ob_flush();
				...
				if( $zz_yykinfo_rtncd != 0 ){
					...
					$log_office_cd = '';			//オフィスコード
					$log_kaiin_no = 'batch';		//会員番号 または スタッフコード
					$log_naiyou = '個別カウンセリング 当日メール送信 が異常終了しました。<br>予約内容の取り込みに失敗しました。予約No[' . sprintf("%05d",$yyk_no[$m]) . ']';	//内容
					...
					//当日送信フラグを 1:当日メール送信済み に更新する
					$query = 'update D_CLASS_YYK set TJT_MAIL_SEND_FLG = 1 where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $zz_yykinfo_office_cd . '" and CLASS_CD = "' . $zz_yykinfo_class_cd . '" and YMD = "' . $zz_yykinfo_ymd . '" and JKN_KBN = "' . $zz_yykinfo_jkn_kbn . '" and YYK_NO = ' . $yyk_no[$m] . ' and KAIIN_ID = "' . $zz_yykinfo_kaiin_id . '"';
					$result = mysql_query($query);
					if (!$result) {
						$err_flg = 4;
						...
						$log_office_cd = '';			//オフィスコード
						$log_kaiin_no = 'batch';		//会員番号 または スタッフコード
						$log_naiyou = 'クラス予約を更新しました。（当日メール送信フラグ  1:当日メール送信済み）';	//内容
						$log_err_inf = $query;	//エラー情報
						require( './zy_log.php' );
						//************
...
//##############################################################
				...
				while( $c < $send_mail_adr_cnt ){
					...
					$subject .= '個別カウンセリングのお知らせ(当日送信)';
			
					// 本文
					$content = $send_kaiin_nm[$c] . " 様\n\n";
					$content .= $Mkanri_meishou . "です。\n";
					$content .= "当協会の個別カウンセリングの予約日が\n";
					$content .= "本日となりましたのでお知らせします。\n\n";
					$content .= "---------------\n";
					$content .= "▼予約内容\n";
					$content .= "---------------\n";
					$content .= "個別カウンセリング\n\n";
		...
		if( $err_flg == 0 ){
			//過去のログインキー発行を削除する（２日前データは削除する）
			$query = 'delete from D_LOGIN_KEY where KG_CD = "' . $DEF_kg_cd . '" and TRK_TIME < "' . $znjt_yyyymmdd . '000000";';
			$result = mysql_query($query);
			if (!$result) {
				$err_flg = 4;
				...
				$log_office_cd = '';			//オフィスコード
				$log_kaiin_no = 'batch';		//会員番号 または スタッフコード
				$log_naiyou = 'ログインキー発行の過去データ削除に失敗しました。';	//内容
				$log_err_inf = $query;	//エラー情報
				require( './zy_log.php' );
				//************
		...
		if( $err_flg == 0 ){
			...
			$log_office_cd = '';			//オフィスコード
			$log_kaiin_no = 'batch';		//会員番号 または スタッフコード
			$log_naiyou = '<font color=blue>個別カウンセリング 当日メール送信 を終了しました。</font><br>対象メンバー数[' . $yyk_cnt . ']件　送信メンバー数[' . $send_mail_cnt . ']件';	//内容
			$log_err_inf = '';	//エラー情報
			require( './zy_log.php' );
			//************
...
print('個別カウンセリング　当日メール送信バッチ end[' . date( "Y-m-d H:i:s", time() ) . ']<br>');
