...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kbt_counseling_kkn.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kbt_counseling_menu.php','kbt_counseling_can1.php','kbt_counseling_select_date.php');
	...
	if( $err_flg != 0 ){
		...
		if( $err_flg == 0 ){
			...
			print('<font size="4">' . $zz_yykinfo_kaiin_nm . '</font>&nbsp;様');
			print('</td>');
			//メニューへ戻るボタン
...
//			print('</tr>');
			...
			if( $zz_yykinfo_soudan != "" ){
				...
				//未登録（スタッフ画面からは未入力可能）
				print('<table border="0"><tr><td width="560" align="center"><img src="./img_' . $lang_cd . '/mitrk.png" border="0"></td></tr></table>');
			}
			print('</td>');
			print('</tr>');
			print('</table>');
			...
			if( $zz_yykinfo_ymd > $now_yyyymmdd2 || ($zz_yykinfo_ymd == $now_yyyymmdd2 && $zz_yykinfo_st_time > (($now_hh * 100) + $now_ii) ) ){
				...
				print('</form>');
				//予約の取消ボタン
				print('<form method="post" action="' . $sv_https_adr . 'yoyaku/kbt_counseling_can1.php#kbtcounseling">');
				print('<input type="hidden" name="prc_gmn" value="' . $gmn_id . '">');
				print('<input type="hidden" name="lang_cd" value="' . $lang_cd . '">');
				print('<input type="hidden" name="yykkey_ang_str" value="' . $yykkey_ang_str . '">');
				print('<input type="hidden" name="kaiin_id" value="' . $kaiin_id . '">');
			...
			//予約が無くなった
			print('<br><font color="red">予約が取消されていますので、始めからお願いします。</font><br><br>');
