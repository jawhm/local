...
//set parameter
//-------------
//	$mailadd 	= 'meminfo@jawhm.or.jp';    /* Khang comment */
//	$mailoffice = 'toiawase@jawhm.or.jp';       /* Khang comment */
//	$ok_url_payment 	= 'http://www.jawhm.or.jp/mem/payment.php'; /* Khang comment */
//	$wrong_url_payment	= 'http://www.jawhm.or.jp/mem/payment.php'; /* Khang comment */

$mailadd = 'khangld@evolableasia.vn';
$mailoffice = 'khangld@evolableasia.vn';
$ok_url_payment = 'http://jawhm.localhost/mem/payment.php';
$wrong_url_payment = 'http://jawhm.localhost/mem/payment.php';
...
if ($act == 's4') {
    ...
    if ($dat_email == '') {
        $dat_email = @$_GET['email'];
    }
    if ($dat_mailcheck == '') {
        $dat_mailcheck = @$_GET['mailcheck'];
    }
    $dat_sid = '';
    $dat_name1 = '';
    $dat_name2 = '';
            ...
            if ($dat_email == $cur_email) {
                if ($dat_mailcheck == $cur_mailcheck || $k <> '') {
                    ...
                    if (!isset($no_store)) {
                        ...
                        $msg_store = 'お支払頂くコンビニエンスストアを選択してください。<br />';
                    }
                } else {
                    // 承認コード不一致
                    $act = 's3';
...
if ($act == 's4,5') {  // use for conbini only, update + send email before displaying page
    if ($conbini_process == 'OK') {
        ...
        $body .= chr(10);
        $body .= 'なお、コンビニでのお支払は以下の有効期限までにお願い致します。';
        $body .= chr(10);
        $body .= chr(10);
...
<link rel="stylesheet" type="text/css"  href="/mem/css/conbini_logo.css" />
<link rel="stylesheet" type="text/css"  href="/mem/css/memberscript.css" />
