...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kanri_class_del2.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kanri_class_del1.php');
	...
	//エラーなし
		...
		if (!$result) {
			...
			if( $row[0] == 0 ){
				...
				$log_office_cd = $office_cd;	//オフィスコード
				$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
				$log_naiyou = '予約種別コードが既に削除されている。<br>オフィスコード[' . $select_office_cd . '] 予約種別コード[' . $class_cd . ']';	//内容
				$log_err_inf = $query;			//エラー情報
				require( './zs_log.php' );
				//************
				...
				print('<font size="5" color="gray">' . $class_cd . '</font>&nbsp;&nbsp;<br>');
				print('</td>');
				...
				if( $delete_flg == 1 ){
					...
					//クラスマスタ
					$query = 'delete from M_CLASS where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $select_office_cd . '" and CLASS_CD = "' . $class_cd . '";';
					$result = mysql_query($query);
					if (!$result) {
						$err_flg = 4;
						//エラーメッセージ表示
						...
						$log_office_cd = $office_cd;	//オフィスコード
						$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
						$log_naiyou = 'クラスマスタの削除に失敗しました。';	//内容
						$log_err_inf = $query;			//エラー情報
						require( './zs_log.php' );
						//************
						...
						$log_office_cd = $office_cd;	//オフィスコード
						$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
						$log_naiyou = 'クラスマスタを削除しました。[' . $select_office_cd . '-' . $class_cd . ']';	//内容
						$log_err_inf = $query;			//エラー情報
						require( './zs_log.php' );
						//************
					...
					//クラス時間割
					$query = 'delete from M_CLASS_JKNWR where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $select_office_cd . '" and CLASS_CD = "' . $class_cd . '";';
					$result = mysql_query($query);
					if (!$result) {
						$err_flg = 4;
						//エラーメッセージ表示
						...
						$log_office_cd = $office_cd;	//オフィスコード
						$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
						$log_naiyou = 'クラス削除によりクラス時間割も削除しました。[' . $select_office_cd . '-' . $class_cd . ']';	//内容
						$log_err_inf = $query;			//エラー情報
						require( './zs_log.php' );
						//************
					...
					if( $err_flg == 0 ){
						...
						$log_office_cd = $office_cd;	//オフィスコード
						$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
						$log_naiyou = '予約種別を削除しました。<br>オフィス[' . $select_office_cd . '] 予約種別[' . $class_cd . ']';	//内容
						$log_err_inf = $query;			//エラー情報
						require( './zs_log.php' );
						//************
						...
						print('<font color="blue">※削除しました。</font><br>');
						...
						print('<td align="left">');
						print('開始日<br>');
						print('<font color="gray" size="5">&nbsp;<b>' . $st_year . '</b></font>');
						print('年');
						print('<font color="gray" size="5">&nbsp;<b>' . $st_month . '</b></font>');
						print('月');
						print('<font color="gray" size="5">&nbsp;<b>' . $st_day . '</b></font>');
						print('日 から');
						print('</td>');
						print('<td align="left">');
						print('終了日<font size="2">&nbsp;(現在の最大日は2037/12/31)</font><br>');
						print('<font color="gray" size="5">&nbsp;<b>' . $ed_year . '</b></font>');
						print('年');
						print('<font color="gray" size="5">&nbsp;<b>' . $ed_month . '</b></font>');
						print('月');
						print('<font color="gray" size="5">&nbsp;<b>' . $ed_day . '</b></font>');
						print('日 まで');
						print('</td>');
						print('</tr>');
						print('</table>');
						...
						print('<b>有効／無効(*)</b><br>');
						if( $yukou_flg == 1 ){
							print('<font color="gray" size="5"><b>有効</b></font>');
						}else{
							print('<font color="gray" size="5"><b>無効</b></font>');
						}
					...
					$query = 'update M_CLASS set YUKOU_FLG = 0,UPDATE_TIME = "' . date("YmdHis") . '",UPDATE_STAFF_CD = "' . $staff_cd . '" where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $select_office_cd . '" and CLASS_CD = "' . $class_cd . '";';
					$result = mysql_query($query);
					if (!$result) {
						$err_flg = 4;
						//エラーメッセージ表示
						...
						$log_office_cd = $office_cd;	//オフィスコード
						$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
						$log_naiyou = 'クラスを無効化しました。[' . $select_office_cd . '-' . $class_cd . ']';	//内容
						$log_err_inf = $query;			//エラー情報
						require( './zs_log.php' );
						//************
						...
						$log_office_cd = $office_cd;	//オフィスコード
						$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
						$log_naiyou = 'クラスを無効化しました。<br>オフィス[' . $select_office_cd . '] 予約種別[' . $class_cd . ']';	//内容
						$log_err_inf = '';				//エラー情報
						require( './zs_log.php' );
						//************
						...
						print('<font color="blue">※無効化しました。</font><br>');
