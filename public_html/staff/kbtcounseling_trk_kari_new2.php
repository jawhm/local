...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kbtcounseling_trk_kari_new2.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kbtcounseling_trk_kari_new1.php');
	...
	//エラーなし
		...
		if( $zz_yykinfo_rtncd == 0 ){
			$select_office_cd = $zz_yykinfo_office_cd;
			$select_ymd = substr($zz_yykinfo_ymd,0,4) . substr($zz_yykinfo_ymd,5,2) . substr($zz_yykinfo_ymd,8,2);
			$select_jknkbn = $zz_yykinfo_jkn_kbn;
			
		}else if( $zz_yykinfo_rtncd == 1 ){
			$err_flg = 4;
...
//			}else{
			if( 1 ){
				...
				$query = 'update D_CLASS_YYK set KAIIN_KBN = 9,KAIIN_ID = "' . $kaiin_id . '",KAIIN_NM = ENCODE("' . $kaiin_nm1 . '　' . $kaiin_nm2 . '","' . $ANGpw . '"),';
				if( $kaiin_kyoumi != "" ){
					$query .= 'KYOUMI = ENCODE("' . $kaiin_kyoumi . '","' . $ANGpw . '"),';	
				}
				if( $kaiin_jiki != "" ){
					$query .= 'JIKI = ENCODE("' . $kaiin_jiki . '","' . $ANGpw . '"),';	
				}
				if( $kaiin_soudan != "" ){
					$query .= 'SOUDAN = ENCODE("' . $kaiin_soudan . '","' . $ANGpw . '"),';	
				}
				$query .= 'YYK_TIME = "' . $now_time . '",CANCEL_TIME = "' . $cancel_ymdhis . '",YYK_STAFF_CD = "' . $staff_cd . '" where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $zz_yykinfo_office_cd . '" and CLASS_CD = "' . $zz_yykinfo_class_cd . '" and YMD = "' . $zz_yykinfo_ymd . '" and JKN_KBN = "' . $zz_yykinfo_jkn_kbn . '" and YYK_NO = ' . $select_yyk_no . ' and KAIIN_ID = "' . $zz_yykinfo_kaiin_id . '";';
				$result = mysql_query($query);
				if (!$result) {
		...
		if( $err_flg == 0 ){
			...
			if( $err_cnt == 0 ){
				...
				$log_office_cd = $office_cd;	//オフィスコード
				$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
				$log_naiyou = '個別カウンセリングを予約しました。（新規顧客）<br>予約No[' . sprintf("%05d",$select_yyk_no) . '] 会場[' . $Moffice_office_nm . '] 日時[' . substr($select_ymd,0,4) . '-' . sprintf("%02d",substr($select_ymd,4,2)) . '-' . sprintf("%02d",substr($select_ymd,6,2)) . ' ' . sprintf("%02d",intval($Mclass_st_time / 100)) . ':' . sprintf("%02d",($Mclass_st_time % 100)) . '-' . sprintf("%02d",intval($Mclass_ed_time / 100)) . ':' . sprintf("%02d",($Mclass_ed_time % 100)) . ']<br>お客様番号[' . $kaiin_id . ']';	//内容
				...
				print('<td width="565" align="center" valign="middle" background="../img_' . $lang_cd . '/bg_pink_565x20.png"><font size="6" color="blue">' . sprintf("%05d",$select_yyk_no) . '</font>&nbsp;&nbsp;<font size="1">(' . $now_time . ')</font></td>');
				print('</tr>');
				//会場
				print('<tr>');
				...
				print('<input type="hidden" name="select_ymd" value="' . substr($zz_yykinfo_ymd,0,4) . substr($zz_yykinfo_ymd,5,2) . substr($zz_yykinfo_ymd,8,2) . '">');
				...
				//お客様番号（本画面では、仮登録のみ）
				print('<tr>');
				print('<td align="center" valign="middle" background="../img_' . $lang_cd . '/bg_yellow_110x20.png"><img src="./img_' . $lang_cd . '/title_mini_okyakusamano.png" border="0"></td>');
				print('<td align="center" valign="middle" background="../img_' . $lang_cd . '/bg_yellow_565x20.png">');
