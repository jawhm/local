...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kanri_staff_ksn2.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kanri_staff_ksn1.php');
	...
	//エラーなし
		...
		$open_stf_nm = $_POST['open_stf_nm'];	//登録対象のスタッフ名
		...
		if (!$result) {
			...
			if( $row[0] == 0 ){
				...
				$log_office_cd = $office_cd;	//オフィスコード
				$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
				$log_naiyou = 'スタッフコードが削除されている。スタッフコード[' . $stf_cd . ']';	//内容
				$log_err_inf = $query;			//エラー情報
				require( './zs_log.php' );
				//************
				...
				print('<font color="red">※以下のスタッフコードは既に削除されています。</font><br>');
				...
				$query = 'update M_STAFF set STAFF_NM = ENCODE("' . $stf_nm . '","' . $ANGpw . '"),OPEN_STAFF_NM = ENCODE("' . $open_stf_nm . '","' . $ANGpw . '"),';
				if( $class_cd1 == '' ){
					$query .= 'CLASS_CD1 = NULL,';
				}else{
					$query .= 'CLASS_CD1 = "' . $class_cd1 . '",';
				}
				if( $class_cd2 == '' ){
					$query .= 'CLASS_CD2 = NULL,';
				}else{
					$query .= 'CLASS_CD2 = "' . $class_cd2 . '",';
				}
				if( $class_cd3 == '' ){
					$query .= 'CLASS_CD3 = NULL,';
				}else{
					$query .= 'CLASS_CD3 = "' . $class_cd3 . '",';
				}
				if( $class_cd4 == '' ){
					$query .= 'CLASS_CD4 = NULL,';
				}else{
					$query .= 'CLASS_CD4 = "' . $class_cd4 . '",';
				}
				if( $class_cd5 == '' ){
					$query .= 'CLASS_CD5 = NULL,';
				}else{
					$query .= 'CLASS_CD5 = "' . $class_cd5 . '",';
				}
				$query .= 'OPE_AUTH = ' . $ope_auth . ',KANRISYA_FLG = ' . $kanri_flg . ',YUKOU_FLG = ' . $yukou_flg . ',ST_DATE = "' . $st_year . sprintf("%02d",$st_month) . sprintf("%02d",$st_day) . '",ED_DATE = "' . $ed_year . sprintf("%02d",$ed_month) . sprintf("%02d",$ed_day) . '",UPDATE_TIME = "' . $now_time . '",UPDATE_STAFF_CD = "' . $staff_cd . '" where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $select_office_cd . '" and STAFF_CD = "' . $stf_cd . '";';
				$result = mysql_query($query);
				if (!$result) {
					$err_flg = 4;
					//エラーメッセージ表示
					...
					$log_office_cd = $office_cd;	//オフィスコード
					$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
					$log_naiyou = 'スタッフマスタを更新しました。[' . $select_office_cd . '-' . $stf_cd . ']';	//内容
					$log_err_inf = $query;			//エラー情報
					require( './zs_log.php' );
					//************
				...
				if (!$result) {
					...
					while( $row = mysql_fetch_array($result) ){
						$Mstfrnr_staff_tel = $row[0];
						$Mstfrnr_staff_mail = $row[1];
						
					}
					if( $stf_tel != $Mstfrnr_staff_tel || $stf_mail != $Mstfrnr_staff_mail ){
						//連絡情報が変わったので更新する
						//文字コード設定（insert/update時に必須）
						require( '../zz_mojicd.php' );
				
						$query = 'update M_STAFF_RNR set ';
						if( $stf_tel == '' ){
							$query .= 'STAFF_TEL = ENCODE(NULL,"' . $ANGpw . '"),';
						}else{
							$query .= 'STAFF_TEL = ENCODE("' . $stf_tel . '","' . $ANGpw . '"),';
						}
						if( $stf_mail == '' ){
							$query .= 'STAFF_MAIL = ENCODE(NULL,"' . $ANGpw . '"),';
						}else{
							$query .= 'STAFF_MAIL = ENCODE("' . $stf_mail . '","' . $ANGpw . '"),';
						}
						$query .= 'UPDATE_TIME = "' . $now_time . '",UPDATE_STAFF_CD = "' . $staff_cd . '" where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $select_office_cd . '" and STAFF_CD = "' . $stf_cd . '";';
						$result = mysql_query($query);
						if (!$result) {
							$err_flg = 4;
							//エラーメッセージ表示
							...
							$log_office_cd = $office_cd;	//オフィスコード
							$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
							$log_naiyou = 'スタッフ連絡情報の更新に失敗しました。';	//内容
							$log_err_inf = $query;			//エラー情報
							require( './zs_log.php' );
							//************
							...
							$log_office_cd = $office_cd;	//オフィスコード
							$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
							$log_naiyou = 'スタッフ連絡情報を更新しました。[' . $select_office_cd . '-' . $stf_cd . ']';	//内容
							$log_err_inf = $query;			//エラー情報
							require( './zs_log.php' );
							//************
				...
				if( $err_flg == 0 ){
					...
					$log_office_cd = $office_cd;	//オフィスコード
					$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
					$log_naiyou = 'スタッフを更新しました。<br>オフィス[' . $Moffice_office_nm . '] スタッフ[' . $stf_nm . ']';	//内容
					$log_err_inf = '';			//エラー情報
					require( './zs_log.php' );
					//************
					...
					//メールアドレス／予約情報フラグ／メール連絡対象期間
					print('<table border="0">');
					print('<tr>');
					print('<td align="left" valign="top"><b>メールアドレス</b>&nbsp;&nbsp;<br>');
					if( $stf_mail == '' ){
						print('<font color="gray">（未登録）</font>');
					}else{
