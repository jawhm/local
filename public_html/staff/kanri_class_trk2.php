...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kanri_class_trk2.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kanri_class_trk1.php');
	...
	//エラーなし
		...
		if (!$result) {
			...
			if( $row[0] > 0 ){
				...
				$log_office_cd = $office_cd;	//オフィスコード
				$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
				$log_naiyou = 'クラスコードが既に登録されている。クラスコード[' . $class_cd . ']';	//内容
				$log_err_inf = $query;			//エラー情報
				require( './zs_log.php' );
				//************
				...
				print('<font color="red">※以下の予約種別コードは既に登録されています。<br>');
				...
				$query = 'insert into M_CLASS values ("' . $DEF_kg_cd . '","' . $select_office_cd . '","' . $class_cd . '",';
				if( $class_nm == '' ){
					$query .= 'NULL,';
				}else{
					$query .= '"' . $class_nm . '",';
				}
				$query .= $yukou_flg . ',"' . $st_date . '","' . $ed_date . '","'.date("YmdHis").'","' . $staff_cd . '");';
				$result = mysql_query($query);
				if (!$result) {
					$err_flg = 4;
					//エラーメッセージ表示
					...
					$log_office_cd = $office_cd;	//オフィスコード
					$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
					$log_naiyou = 'クラスマスタの登録に失敗しました。';	//内容
					$log_err_inf = $query;			//エラー情報
					require( './zs_log.php' );
					//************
					...
					$log_office_cd = $office_cd;	//オフィスコード
					$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
					$log_naiyou = 'クラスマスタを登録しました。[' . $select_office_cd . '-' . $class_cd . ']';	//内容
					$log_err_inf = $query;			//エラー情報
					require( './zs_log.php' );
					//************
					...
					$log_office_cd = $office_cd;	//オフィスコード
					$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
					$log_naiyou = '予約種別を登録しました。<br>オフィス[' . $select_office_cd . '] 予約種別[' . $class_cd . ']';	//内容
					$log_err_inf = $query;			//エラー情報
					require( './zs_log.php' );
					//************
