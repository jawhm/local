...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kanri_eigyojkn_top.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kanri_top.php','kanri_eigyojkn_select.php','kanri_eigyojkn_trk0.php','kanri_eigyojkn_trk1.php','kanri_eigyojkn_trk2.php',
	...
	//エラーなし
		...
		print('<tr>');
		print('<td width="815" align="center"><font color="blue">※営業時間を設定するオフィスを指定してください。</font></td>');
		...
		if( $err_flg == 0 ){
			...
			print('<form name="form1" method="post" action="kanri_eigyojkn_select.php">');
			print('<input type="hidden" name="prc_gmn" value="' . $gmn_id . '">');
			print('<input type="hidden" name="lang_cd" value="' . $lang_cd . '" />');
			print('<input type="hidden" name="office_cd" value="' . $office_cd . '" />');
			print('<input type="hidden" name="staff_cd" value="' . $staff_cd . '">');
			...
			print('<table  border="0">');
			print('<tr><td width="250" slign="left"><img src="./img_' . $lang_cd . '/bar_officenm2.png"></td>');
