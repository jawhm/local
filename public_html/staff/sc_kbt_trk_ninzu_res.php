...
<title>スケジュール－個別カウンセリング受付人数登録（結果）</title>
...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'sc_kbt_trk_ninzu_res.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('sc_kbt_trk_ninzu.php','sc_kbt_trk_ninzu_res.php');
	...
	//エラーなし
		...
		//前週の最終日
		$bfweek_date = date("Ymd", mktime(0, 0, 0, sprintf("%d",substr($target_yyyymmdd,4,2)), (sprintf("%d",substr($target_yyyymmdd,6,2)) - 1) , sprintf("%d",substr($target_yyyymmdd,0,4))) );
		...
		while( $t < 7 ){
			...
			//引数取得および現在登録されている受付人数を取得する
			$name = "ninzu_ikt" . $t;
			$tmp_ninzu_itk = $_POST[$name];
			
			$j = 0;
			while( $j < $Mclassjknwr_cnt ){
				
				$name = "ninzu" . $t . $Mclassjknwr_jkn_kbn[$j];
				$tmp_ninzu = $_POST[$name];
				
				if( $tmp_ninzu == "" && $tmp_ninzu_itk != "" ){
					$tmp_ninzu = $tmp_ninzu_itk;
				}
				
				if( $tmp_ninzu != "" ){
					//該当日／時間割の予約数を求める
					$query = 'select count(*) from D_CLASS_YYK where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $select_office_cd . '" and CLASS_CD = "KBT" and YMD = "' . $target_yyyymmdd . '" and CLASS_CD = "KBT" and JKN_KBN = "' . $Mclassjknwr_jkn_kbn[$j] . '";';
					$result = mysql_query($query);
					if (!$result) {
						$err_flg = 4;
						//エラーメッセージ表示
		...
		if( $err_cnt == 0 ){
			...
			while( $t < 7 && $err_flg == 0 ){
				...
				$j = 0;
				while( $j < $Mclassjknwr_cnt && $err_flg == 0 ){
					...
					//既存データの存在有無チェック
					$query = 'select count(*) from D_STAFF_SC where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $select_office_cd . '" and YMD = "' . $target_yyyymmdd . '" and CLASS_CD = "KBT" and JKN_KBN = "' . $Mclassjknwr_jkn_kbn[$j] . '";';
					$result = mysql_query($query);
					if (!$result) {
						$err_flg = 4;
						//エラーメッセージ表示
						...
						$row = mysql_fetch_array($result);
						if( $row[0] == 0 ){
							//未登録なので、スタッフスケジュールに登録(INSERT)する
							$query = 'insert into D_STAFF_SC values("' . $DEF_kg_cd . '","' . $select_office_cd . '","' . $target_yyyymmdd . '","KBT","' . $Mclassjknwr_jkn_kbn[$j] . '",';
							if( $tmp_ninzu == "" ){
								$query .= 'NULL,';
							}else{
								$query .= $tmp_ninzu . ',';
							...
							if (!$result) {
								...
								$log_office_cd = $office_cd;	//オフィスコード
								$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
								$log_naiyou = 'スタッフスケジュールを登録しました。オフィス[' . $select_office_cd . '] 年月日[' . $target_yyyymmdd . '] 予約種別[KBT] 時間区分[' . $Mclassjknwr_jkn_kbn[$j] . ']';	//内容
								$log_err_inf = $query;			//エラー情報
								require( './zs_log.php' );
								//************
							...
							//データ存在なのでスタッフスケジュールを更新する
							$query = 'update D_STAFF_SC set ';
							if( $tmp_ninzu == "" ){
								$query .= 'UKTK_NINZU = NULL ';
							}else{
								$query .= 'UKTK_NINZU = ' . $tmp_ninzu . ' ';
							}
							$query .= 'where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $select_office_cd . '" and YMD = "' . $target_yyyymmdd . '" and CLASS_CD = "KBT" and JKN_KBN = "' . $Mclassjknwr_jkn_kbn[$j] . '";';
							$result = mysql_query($query);
							if (!$result) {
								$err_flg = 4;
								//エラーメッセージ表示
								...
								$log_office_cd = $office_cd;	//オフィスコード
								$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
								$log_naiyou = 'スタッフスケジュールを更新しました。オフィス[' . $select_office_cd . '] 年月日[' . $target_yyyymmdd . '] 予約種別[KBT] 時間区分[' . $Mclassjknwr_jkn_kbn[$j] . ']';	//内容
								$log_err_inf = $query;			//エラー情報
								require( './zs_log.php' );
								//************
				...
				if( $t == 6 ){
					$last_date = $target_yyyymmdd;	//選択週の最終日
				}
		...
		if( $err_flg == 0 && $err_cnt == 0 ){
			...
			$log_office_cd = $office_cd;	//オフィスコード
			$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
			$log_naiyou = '個別カウンセリング（受付人数）を登録しました。<br>オフィス[' . $Moffice_office_nm . '] 登録期間[' . $target_yyyymmdd . '-' . $last_date . ']';	//内容
			$log_err_inf = '';			//エラー情報
			require( './zs_log.php' );
			//************
			...
			print('<tr>');
			//前週へ
			print('<form method="post" action="sc_kbt_trk_ninzu.php">');
			print('<input type="hidden" name="prc_gmn" value="' . $gmn_id . '">');
			print('<input type="hidden" name="lang_cd" value="' . $lang_cd . '" />');
			print('<input type="hidden" name="office_cd" value="' . $office_cd . '" />');
			print('<input type="hidden" name="staff_cd" value="' . $staff_cd . '">');
			...
			print('<td width="150" align="center" valihn="middle">');
			$tabindex++;
			...
			print('</form>');
			//「登録しました」
			print('<td width="250" align="center" valihn="middle">');
			print('<img src="./img_' . $lang_cd . '/title_trk_ok.png" border="0">');
			print('</td>');
			//翌週へ
			print('<form method="post" action="sc_kbt_trk_ninzu.php">');
			print('<input type="hidden" name="prc_gmn" value="' . $gmn_id . '">');
			...
			while( $t < 7 ){
				...
				print('<tr>');
				print('<td width="80" align="left" valign="bottom"><img src="./img_' . $lang_cd . '/yyyy_' . substr($target_yyyymmdd,0,4) . '_black.png" border="0"></td>');	//年
				...
				while( $j < $Mclassjknwr_cnt ){
					...
					if( !($target_eigyou_st_time <= $Mclassjknwr_st_time[$j] && $Mclassjknwr_ed_time[$j] <= $target_eigyou_ed_time) ){
						//営業時間外
						print('<img src="../img_' . $lang_cd . '/title_eigyou_jkngai.png" border="0">');	//営業時間外
						
					}else{
						//営業時間内
						if( $tmp_uktk_ninzu != "" ){
							print('<font size="5" color="blue">' . $tmp_uktk_ninzu . '</font>&nbsp;名');
						}else{
							//未登録
