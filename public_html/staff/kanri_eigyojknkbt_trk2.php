...
<title>管理画面－営業時間個別（新規登録）結果</title>
...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kanri_eigyojknkbt_trk2.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kanri_eigyojknkbt_trk1.php');
	...
	//エラーなし
		...
		if (!$result) {
			...
			if( $row[0] > 0 ){
				...
				//エラーメッセージ表示
				print('<br><font color="red">エラー：入力された対象日の営業時間は既に登録されています。オフィスコード[' . $select_office_cd . '] 対象日[' . $select_date . ']</font><br><br>');
				print('<hr>');
				...
				$log_office_cd = $office_cd;	//オフィスコード
				$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
				$log_naiyou = '営業時間個別が既に登録されている。オフィスコード[' . $select_office_cd . '] 対象日[' . $select_date . ']';	//内容
				$log_err_inf = $query;			//エラー情報
				require( './zs_log.php' );
				//************
				...
				//営業時間個別の登録
				$query = 'insert into D_EIGYOJKN_KBT values ("' . $DEF_kg_cd . '","' . $select_office_cd . '","' . $select_date . '",' . $tnp_st_time . ',' . $tnp_ed_time . ',"' . date("YmdHis") . '","' . $staff_cd . '");';
				$result = mysql_query($query);
				if (!$result) {
					$err_flg = 4;
					//エラーメッセージ表示
					...
					$log_office_cd = $office_cd;	//オフィスコード
					$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
					$log_naiyou = '営業時間個別の登録に失敗しました。';	//内容
					$log_err_inf = $query;			//エラー情報
					require( './zs_log.php' );
					//************
					...
					$log_office_cd = $office_cd;	//オフィスコード
					$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
					$log_naiyou = '営業時間個別を登録しました。オフィス[' . $select_office_cd . '] 対象日[' . $select_date . ']';	//内容
					$log_err_inf = $query;			//エラー情報
					require( './zs_log.php' );
					//************
				...
				if( $err_flg == 0 ){
					...
					$log_office_cd = $office_cd;	//オフィスコード
					$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
					$log_naiyou = '営業時間個別を登録しました。<br>オフィス[' . $select_office_cd . '] 対象日[' . $select_date . ']';	//内容
					$log_err_inf = '';			//エラー情報
					require( './zs_log.php' );
					//************
