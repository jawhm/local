...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kanri_eigyojkn_ksn2.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kanri_eigyojkn_ksn1.php');
	...
	$select_office_cd = $_POST['select_office_cd'];	//変更対象のオフィスコード
	...
	//エラーなし
		...
		if( $err_flg == 0 ){
			...
			//更新前のデータを削除する
			//営業時間マスタ
			$query = 'delete from M_EIGYOJKN where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $select_office_cd . '" and ST_DATE = "' . $select_st_date . '";';
			$result = mysql_query($query);
			if (!$result) {
				...
				$log_office_cd = $office_cd;	//オフィスコード
				$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
				$log_naiyou = '営業時間マスタの削除に失敗しました。';	//内容
				$log_err_inf = $query;			//エラー情報
				require( './zs_log.php' );
				//************
				...
				$log_office_cd = $office_cd;	//オフィスコード
				$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
				$log_naiyou = '営業時間の変更のため、旧データを削除する。';	//内容
				$log_err_inf = $query;			//エラー情報
				require( './zs_log.php' );
				//************
			...
			while( $i < $eigyo_query_cnt && $err_flg == 0 ){
				...
				if (!$result) {
					...
					$log_office_cd = $office_cd;	//オフィスコード
					$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
					$log_naiyou = '営業時間の変更データを登録しました。オフィス[' . $select_office_cd . '] 新開始日[' . $st_date . ']';	//内容
					$log_err_inf = $query;			//エラー情報
					require( './zs_log.php' );
					//************
			...
			if( $err_flg == 0 ){
				...
				$log_office_cd = $office_cd;	//オフィスコード
				$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
				$log_naiyou = '営業時間を更新しました。<br>オフィス[' . $select_office_cd . '] 旧開始日[' . $select_st_date . '] 新開始日[' . $st_date . ']';	//内容
				$log_err_inf = $query;			//エラー情報
				require( './zs_log.php' );
				//************
