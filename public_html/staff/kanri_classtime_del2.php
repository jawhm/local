...
<title>管理画面－予約種別時間割（削除）結果</title>
...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kanri_classtime_del2.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kanri_classtime_del1.php');
	...
	//エラーなし
		...
		if( $err_flg == 0 ){
			...
			if (!$result) {
				...
				$log_office_cd = $office_cd;	//オフィスコード
				$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
				$log_naiyou = 'クラス時間割を削除しました。オフィス[' . $select_office_cd . '] クラス[' . $select_class_cd . '] 開始日[' . $select_st_date . ']';	//内容
				$log_err_inf = $query;			//エラー情報
				require( './zs_log.php' );
				//************
				...
				$log_office_cd = $office_cd;	//オフィスコード
				$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
				$log_naiyou = '予約種別時間割を削除しました。<br>オフィス[' . $select_office_cd . '] 予約種別[' . $select_class_cd . '] 開始日[' . $select_st_date . ']';	//内容
				$log_err_inf = $query;			//エラー情報
				require( './zs_log.php' );
				//************
...
//				print('<b>有効期間(*)</b>・・・予約種別時間割の有効期間<br>');
				...
				print('<td align="left">');
				print('開始日<br>');
				print('<font color="lightgrey" size="5">&nbsp;<b>' . $st_year . '</b></font>');
				print('年');
				print('<font color="lightgrey" size="5">&nbsp;<b>' . $st_month . '</b></font>');
				print('月');
				print('<font color="lightgrey" size="5">&nbsp;<b>' . $st_day . '</b></font>');
				print('日 から');
				print('</td>');
				print('<td align="left">');
				print('終了日<font size="2"><br>');
				print('<font color="lightgrey" size="5">&nbsp;<b>' . $ed_year . '</b></font>');
				print('年');
				print('<font color="lightgrey" size="5">&nbsp;<b>' . $ed_month . '</b></font>');
				print('月');
				print('<font color="lightgrey" size="5">&nbsp;<b>' . $ed_day . '</b></font>');
				print('日 まで');
				print('</td>');
				print('</tr>');
				print('</table>');
