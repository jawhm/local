...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kanri_eigyojknkbt_del2.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kanri_eigyojknkbt_del1.php');
	...
	$tnp_st_time = $_POST['tnp_st_time'];	//開店時間
	$tnp_ed_time = $_POST['tnp_ed_time'];	//閉店時間
	...
	//エラーなし
		...
		if( $err_flg == 0 ){
			//データを削除する
			//営業時間個別
			$query = 'delete from D_EIGYOJKN_KBT where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $select_office_cd . '" and YMD = "' . $select_date . '";';
			$result = mysql_query($query);
			if (!$result) {
				$err_flg = 4;
				//エラーメッセージ表示
				...
				$log_office_cd = $office_cd;	//オフィスコード
				$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
				$log_naiyou = '営業時間個別の削除に失敗しました。';	//内容
				$log_err_inf = $query;			//エラー情報
				require( './zs_log.php' );
				//************
				...
				$log_office_cd = $office_cd;	//オフィスコード
				$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
				$log_naiyou = '営業時間個別を削除しました。オフィス[' . $select_office_cd . '] 対象日[' . $select_date . ']';	//内容
				$log_err_inf = $query;			//エラー情報
				require( './zs_log.php' );
				//************
		...
		if( $err_flg == 0 ){
			...
			$log_office_cd = $office_cd;	//オフィスコード
			$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
			$log_naiyou = '営業時間個別を削除しました。<br>オフィス[' . $select_office_cd . '] 対象日[' . $select_date . ']';	//内容
			$log_err_inf = $query;			//エラー情報
			require( './zs_log.php' );
			//************
			...
			print('<font size="5" color="gray">' . sprintf("%d",$div) . ':' . sprintf("%02d",$mod) . '</font>');
			print('</td>');
