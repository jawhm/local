...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kanri_class_trk1.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kanri_class_trk0.php','kanri_class_trk1.php');
	...
	//エラーなし
		...
		if( $err_flg == 0 ){
			...
			if( $err_cnt == 0 ){
				...
				print('<form method="post" action="kanri_class_trk0.php">');
				...
				print('</td>');
				print('<td align="right">');
				print('<form method="post" action="kanri_class_trk2.php">');
				print('<input type="hidden" name="prc_gmn" value="' . $gmn_id . '">');
				print('<input type="hidden" name="lang_cd" value="' . $lang_cd . '" />');
				print('<input type="hidden" name="office_cd" value="' . $office_cd . '" />');
				print('<input type="hidden" name="staff_cd" value="' . $staff_cd . '">');
				...
				print('<td><b>予約種別コード(*)</b><br>');
				$tabindex++;
				print('<input type="text" name="class_cd" maxlength="4" size="6" tabindex="' . $tabindex . '" value="' . $class_cd . '" class="');
				if( $err_class_cd == 0 ){
					print('normal');
				}else{
					print('err');
				...
				print('<td><b>予約種別名(*)</b><br>');
				$tabindex++;
				print('<input type="text" name="class_nm" maxlength="40" size="25" tabindex="' . $tabindex . '" value="' . $class_nm . '" class="');
				if( $err_class_nm == 0 ){
					print('normal');
				}else{
					print('err');
				}
				print('" style="font-size:20pt;ime-mode:active;" class="normal" onfocus="this.style.backgroundColor=\'#cccccc\'" onblur="this.style.backgroundColor=\'#E0FFFF\'"><br>');
				print('<font size="2">(全角・半角英数字)</font>');
				print('</td>');
				print('</tr>');
				print('</table>');
