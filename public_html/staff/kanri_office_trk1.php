...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kanri_office_trk1.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kanri_office_trk0.php','kanri_office_trk1.php');
	...
	//エラーなし
		...
		if( $err_cnt == 0 ){
			...
			$query = 'select distinct(ST_DATE),ED_DATE from M_OFFICE where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $select_tenpo_cd . '" order by ST_DATE;';
		...
		if( $err_cnt == 0 ){
			...
			print('<tr>');
			print('<td width="815" align="center"><font color="blue">※以下の内容で登録します。よろしければ登録ボタンを押下してください。</font><br><font color="red" size="2">（まだ登録されていません。）</font></td>');
			print('<form method="post" action="kanri_office_trk0.php">');
			...
			print('</td>');
			print('<td align="right">');
			print('<form method="post" action="kanri_office_trk2.php">');
			print('<input type="hidden" name="prc_gmn" value="' . $gmn_id . '">');
			print('<input type="hidden" name="lang_cd" value="' . $lang_cd . '" />');
			print('<input type="hidden" name="office_cd" value="' . $office_cd . '" />');
			print('<input type="hidden" name="staff_cd" value="' . $staff_cd . '">');
		...
		//確認ＮＧフォーマット
			...
			print('<tr>');
			print('<td width="815" align="left">&nbsp;</td>');
			print('<form method="post" action="kanri_tenpo_top.php">');
			print('<td align="right">');
			print('<input type="hidden" name="prc_gmn" value="' . $gmn_id . '">');
