...
<title>管理画面－予約種別時間割（更新）結果</title>
...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kanri_classtime_ksn2.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kanri_classtime_ksn1.php');
	...
	//エラーなし
		...
		while( $i < 48 ){
			$name = 'jyoren' . $i;
			$jyoren[$i] = $_POST[$name];	//常連フラグ　0：チェックなし 1：チェックあり
			$name = 'jkn_kbn' . $i;
			$jkn_kbn[$i] = $_POST[$name];	//時間区分
			$name = 'st_time' . $i;
		...
		if( $err_flg == 0 ){
			...
			//更新前のデータを削除する
			$query = 'delete from M_CLASS_JKNWR where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $select_office_cd . '" and CLASS_CD = "' . $select_class_cd . '" and ST_DATE = "' . $select_st_date . '";';
			...
			if (!$result) {
				...
				$log_office_cd = $office_cd;	//オフィスコード
				$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
				$log_naiyou = 'クラス時間割の削除に失敗しました。';	//内容
				$log_err_inf = $query;			//エラー情報
				require( './zs_log.php' );
				//************
				...
				$log_office_cd = $office_cd;	//オフィスコード
				$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
				$log_naiyou = 'クラス時間割を更新するため、旧データを削除しました。オフィス[' . $select_office_cd . '] クラス[' . $select_class_cd . '] 開始日[' . $select_st_date . ']';	//内容
				$log_err_inf = $query;			//エラー情報
				require( './zs_log.php' );
				//************
			...
			$i = 0;
			while( $i < 48 && $err_cnt == 0){
				if( $jkn_kbn[$i] != ''){
					
					//文字コード設定（insert/update時に必須）
					require( '../zz_mojicd.php' );
				
					$query = 'insert into M_CLASS_JKNWR values ("' . $DEF_kg_cd . '","' . $select_office_cd . '","' . 
							  $select_class_cd . '",' .	($i + 1 ) . ',"' . $jkn_kbn[$i] . '","' . $st_date . '","' .
							  $ed_date . '",' . $st_time[$i] . ',' . $ed_time[$i] . ',' . $yukou_flg . ',"' .
							  $now_time . '","' . $staff_cd . '")';
					...
					if (!$result) {
						...
						$log_office_cd = $office_cd;	//オフィスコード
						$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
						$log_naiyou = 'クラス時間割の登録に失敗しました。';	//内容
						$log_err_inf = $query;			//エラー情報
						require( './zs_log.php' );
						//************
						...
						$log_office_cd = $office_cd;	//オフィスコード
						$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
						$log_naiyou = 'クラス時間割を更新するため、新データを登録しました。オフィス[' . $select_office_cd . '] クラス[' . $select_class_cd . '] 開始日[' . $st_date . '] 通番[' . ($i + 1 ) . '] 時間区分[' . $jkn_kbn[$i] . ']';	//内容
						$log_err_inf = $query;			//エラー情報
						require( './zs_log.php' );
						//************
			...
			if( $err_cnt == 0 ){
				...
				$log_office_cd = $office_cd;	//オフィスコード
				$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
				$log_naiyou = 'クラス時間割を更新しました。オフィス[' . $select_office_cd . '] クラス[' . $select_class_cd . '] 開始日[' . sprintf("%04d",$st_year) . '-' . sprintf("%02d",$st_month) . '-' . sprintf("%02d",$st_day) . ']';	//内容
				$log_err_inf = $query;			//エラー情報
				require( './zs_log.php' );
				//************
				...
				print('<font color="blue">※更新しました。</font><br><br>');
				...
				while( $i < 16 ){	//16x3
					...
					print('</td>');
					print('<td align="center" background="../img_' . $lang_cd . '/' . $bgfile . '_80x20.png"><font size="5" color="blue">' . $st_time[$i] . '</font></td>');
					print('<td align="center" background="../img_' . $lang_cd . '/' . $bgfile . '_80x20.png"><font size="5" color="blue">' . $ed_time[$i] . '</font></td>');
					//２列目（１７～３２）
					...
					print('</td>');
					print('<td align="center" background="../img_' . $lang_cd . '/' . $bgfile . '_80x20.png"><font size="5" color="blue">' . $st_time[($i + 16)] . '</font></td>');
					print('<td align="center" background="../img_' . $lang_cd . '/' . $bgfile . '_80x20.png"><font size="5" color="blue">' . $ed_time[($i + 16)] . '</font></td>');
					//３列目（３３～４８）
					...
					print('<td align="center" background="../img_' . $lang_cd . '/' . $bgfile . '_80x20.png"><font size="5" color="blue">' . $st_time[($i + 32)] . '</font></td>');
					print('<td align="center" background="../img_' . $lang_cd . '/' . $bgfile . '_80x20.png"><font size="5" color="blue">' . $ed_time[($i + 32)] . '</font></td>');
					print('</tr>');
					$i++;
				}
				print('</table>');
				...
				print('<font color="red">※更新に失敗しました。</font><br>');
