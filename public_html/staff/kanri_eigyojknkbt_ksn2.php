...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kanri_eigyojknkbt_ksn2.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kanri_eigyojknkbt_ksn1.php');
	...
	//エラーなし
		...
		//営業時間個別の更新
		$query = 'update D_EIGYOJKN_KBT set OFFICE_ST_TIME = ' . $tnp_st_time . ',OFFICE_ED_TIME = ' . $tnp_ed_time . ',UPDATE_TIME = "' . date("YmdHis") . '",UPDATE_STAFF_CD = "' .  $staff_cd . '" where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $select_office_cd . '" and YMD = "' . $select_date . '";';
		$result = mysql_query($query);
		if (!$result) {
			$err_flg = 4;
			//エラーメッセージ表示
			...
			$log_office_cd = $office_cd;	//オフィスコード
			$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
			$log_naiyou = '営業時間個別の更新に失敗しました。';	//内容
			$log_err_inf = $query;			//エラー情報
			require( './zs_log.php' );
			//************
			...
			$log_office_cd = $office_cd;	//オフィスコード
			$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
			$log_naiyou = '営業時間個別を更新しました。オフィス[' . $select_office_cd . '] 対象日[' . $select_date . ']';	//内容
			$log_err_inf = $query;			//エラー情報
			require( './zs_log.php' );
			//************
		...
		if( $err_flg == 0 ){
			...
			$log_office_cd = $office_cd;	//オフィスコード
			$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
			$log_naiyou = '営業時間個別を更新しました。<br>オフィス[' . $select_office_cd . '] 対象日[' . $select_date . ']';	//内容
			$log_err_inf = '';			//エラー情報
			require( './zs_log.php' );
			//************
