...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kaiin_img_del2.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kaiin_img_del1.php');
	...
	//エラーなし
		...
		//画像事前読み込み
		print('<img src="./img_' . $lang_cd . '/btn_gonyuryoku_2.png" width="0" height="0" style="visibility:hidden;">');
		...
		if( !$conn_id ){
			...
			if( !$login_result ){
				...
				//既に会員写真があった場合は削除する
				$tmp_file_del = './' . $dir_kaiin_img . '/' . $select_kaiin_no . '.jpeg';
				if( file_exists( $tmp_file_del ) ){
					//画像削除
					$tmp_file_del = './httpdocs/staff/' . $dir_kaiin_img . '/' . $select_kaiin_no . '.jpeg';
		...
		if( $file_err_flg == 0 ){
			...
			$log_naiyou = '会員写真を削除しました。お客様番号[' . $select_kaiin_no . ']';	//内容
			...
			//「写真を削除しました。」
			print('<img src="./img_' . $lang_cd . '/title_fhoto_del_ok.png" border="0"><br>');
			...
			if( $file_err_flg == 1 ){
				...
				$log_office_cd = $office_cd;	//オフィスコード
				$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
				$log_naiyou = '会員写真の削除に失敗しました。お客様番号[' . $select_kaiin_no . ']';	//内容
				$log_err_inf = $query;			//エラー情報
				require( './zs_log.php' );
				//************
			...
			if( $file_err_flg == 1 ){
				//「削除に失敗しました。」
				print('<img src="./img_' . $lang_cd . '/title_fhoto_del_ng.png" border="0"><br>');
			}
