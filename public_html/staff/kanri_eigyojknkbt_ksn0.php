...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kanri_eigyojknkbt_ksn0.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kanri_eigyojkn_select.php','kanri_eigyojknkbt_ksn1.php','kanri_eigyojknkbt_del1.php');
	...
	$select_month = sprintf("%d",$_POST['select_month']);	//選択月
	$select_day = sprintf("%d",$_POST['select_day']);		//選択日
	...
	//エラーなし
		...
		//ページ編集
		//初期値セット
		if( $prc_gmn == 'kanri_eigyojkn_select.php' || $prc_gmn == 'kanri_eigyojknkbt_del1.php' ){

			// 営業時間個別の情報取得
			$query = 'select OFFICE_ST_TIME,OFFICE_ED_TIME from D_EIGYOJKN_KBT where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $select_office_cd . '" and YMD = "' . $select_date . '";';
			...
			if (!$result) {
				...
				$tnp_st_time = $row[0];	//開店時刻
				$tnp_ed_time = $row[1];	//開店時刻
			}
		...
		if( $err_flg == 0 ){
			...
			if( $lock_flg == 0 ){
				print('<font color="blue">※更新後、登録ボタンを押下してください。</font><br>');
			}else{
				print('<font color="red">※対象日に予約データが存在するため、更新および削除はできません。</font><br>');
			}
			
			print('<form name="form2" method="post" action="kanri_eigyojknkbt_ksn1.php">');
			print('<input type="hidden" name="prc_gmn" value="' . $gmn_id . '">');
			print('<input type="hidden" name="lang_cd" value="' . $lang_cd . '" />');
			print('<input type="hidden" name="office_cd" value="' . $office_cd . '" />');
			print('<input type="hidden" name="staff_cd" value="' . $staff_cd . '">');
			...
			print('<tr>');
			print('<td width="815" align="left">&nbsp;</td>');
			print('<form method="post" action="kanri_eigyojknkbt_del1.php">');
			print('<td align="right">');
			print('<input type="hidden" name="prc_gmn" value="' . $gmn_id . '">');
			print('<input type="hidden" name="lang_cd" value="' . $lang_cd . '" />');
			print('<input type="hidden" name="office_cd" value="' . $office_cd . '" />');
