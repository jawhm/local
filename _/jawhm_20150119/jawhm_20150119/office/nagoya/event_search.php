...
//		$rs = $db->query('SELECT id, year(hiduke) as yy, month(hiduke) as mm, day(hiduke) as dd, title, memo, place, k_use, k_title1, k_desc1, k_stat FROM event_list WHERE k_use = 1 AND hiduke >= "'.date("Y/m/d",strtotime("-1 week")).'"  ORDER BY hiduke, id');
		...
		while($row = $rs->fetch(PDO::FETCH_ASSOC)){
			...
			if ($row['place'] == 'nagoya')	{
				...
				if ($row['free'] == 1)	{
					if ($mem_id == '')	{
						...
						if ($row['k_stat'] == 2)	{
							...
							if ($row['booking'] >= $row['pax'])	{
								$c_btn	= '<input class="button_yoyaku" type="button" value="キャンセル待ち" onclick="fnc_yoyaku(this);" title="[名古屋N]'.$title.'" uid="'.$row['id'].'">';
							}else{
								$c_btn	= '<input class="button_yoyaku" type="button" value="メンバー専用予約" onclick="fnc_yoyaku(this);" title="[名古屋N]'.$title.'" uid="'.$row['id'].'">';
							}
						}
					}
				}else{
					if ($row['k_stat'] == 2)	{
						...
						if ($row['booking'] >= $row['pax'])	{
							...
							$c_btn	= '<input class="button_yoyaku" type="button" value="予約" onclick="fnc_yoyaku(this);" title="[名古屋N]'.$title.'" uid="'.$row['id'].'">';
						}
					}
				}
				...
				$c_msg  = '<tr><td nowrap style="vertical-align:top;">'.$row['starttime'].'～ <img src="images/sa03.jpg"><a target="_blank" href="/event/map/?p=nagoya">名古屋会場</a></td><td nowrap style="vertical-align:top;">'.$c_btn.'</td>';
...
<span style="font-size:11pt;">
	<a href="/seminar">名古屋以外での無料セミナーはこちらからどうぞ</a>
</span>
