...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kanri_staff_ksn1.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kanri_staff_ksn0.php','kanri_staff_ksn1.php');
	...
	//エラーなし
		...
		if( $err_flg == 0 ){
			...
			if( $err_cnt == 0 ){
				...
				print('</td>');
				print('<td align="right">');
				print('<form method="post" action="kanri_staff_ksn2.php">');
				print('<input type="hidden" name="prc_gmn" value="' . $gmn_id . '">');
				print('<input type="hidden" name="lang_cd" value="' . $lang_cd . '" />');
				print('<input type="hidden" name="office_cd" value="' . $office_cd . '" />');
				print('<input type="hidden" name="staff_cd" value="' . $staff_cd . '">');
				...
				if( $err_stf_cd == 3 ){
					print('<font size="2" color="red">エラー：既に削除されています。&nbsp;</font>');
				}else{
