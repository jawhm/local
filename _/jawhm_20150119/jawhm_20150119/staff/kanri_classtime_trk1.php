...
<title>管理画面－予約種別時間割（新規登録）確認</title>
...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kanri_classtime_trk1.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kanri_classtime_trk0.php','kanri_classtime_trk1.php');
	...
	//エラーなし
		...
		while( $i < 48 ){
			...
			if( $jyoren[$i] == 1 || $st_time[$i] != '' || $ed_time[$i] != '' ){
				...
				//開始時刻と終了時刻の比較
				if( $err_st_time[$i] == 0 && $err_ed_time[$i] == 0 ){
		...
		if( $err_sts == 0 ){
			...
			$query = 'select distinct(ST_DATE),ED_DATE from M_CLASS_JKNWR where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $select_office_cd . '" and CLASS_CD ="' . $select_class_cd . '" order by ST_DATE;';
					...
					while( $j < $mdata_ttl_cnt ){
						if( $st_time[$i] < $mdata_st_time[$j] ){
							...
							while( $k != $j ){
								...
								$mdata_err_st_time[$k] = $mdata_err_st_time[($k - 1)];
								$mdata_err_ed_time[$k] = $mdata_err_ed_time[($k - 1)];
								$k--;
							}
							//間に入れる
							...
							$mdata_err_st_time[$j] = $err_st_time[$i];
							$mdata_err_ed_time[$j] = $err_ed_time[$i];
							$mdata_ttl_cnt++;
							$set_flg = 1;
							$j =  $mdata_ttl_cnt;
						}else{
		...
		if( $err_flg == 0 ){
			...
			if( $err_sts == 0 ){
				...
				print('<form method="post" action="kanri_classtime_trk0.php">');
				...
				print('<form method="post" action="kanri_classtime_trk2.php">');
				print('<input type="hidden" name="prc_gmn" value="' . $gmn_id . '">');
				print('<input type="hidden" name="lang_cd" value="' . $lang_cd . '" />');
				print('<input type="hidden" name="office_cd" value="' . $office_cd . '" />');
				print('<input type="hidden" name="staff_cd" value="' . $staff_cd . '">');
