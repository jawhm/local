...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kanri_office_ksn2.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kanri_office_ksn1.php');
	...
	$select_office_cd = $_POST['select_office_cd'];	//変更対象の店舗コード
	...
	//エラーなし
		...
		if( $err_flg == 0 ){
			...
			$query = 'update M_OFFICE set OFFICE_NM = "' . $select_office_nm . '",OFFICE_PW = ENCODE("' . $office_pw . '","' . $ANGpw . '"),MAIL_ADR = ENCODE("' . $mail_adr . '","' . $ANGpw . '"),TEL = ENCODE("' . $tel . '","' . $ANGpw . '"),CANCEL_YK_JKN = ' . $cancel_yk_jkn . ',CANCEL_MK_KKN = ' .$cancel_mk_kkn . ',START_YOUBI = ' . $start_youbi . ',TIME_DISP_FLG = ' . $time_disp_flg . ',';
			if( $bikou == '' ){
				$query = $query . 'BIKOU = ENCODE(NULL,"' . $ANGpw . '"),';
			}else{
				$query = $query . 'BIKOU = ENCODE("' . $bikou . '","' . $ANGpw . '"),';
			}
			$query = $query . 'YUKOU_FLG = ' . $yukou_flg . ',ST_DATE = "' . $st_date . '",ED_DATE = "' . $ed_date . '",UPDATE_TIME = "'.date("YmdHis").'",UPDATE_STAFF_CD = "' . $staff_cd . '" where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $select_office_cd . '";';
			...
			if (!$result) {
				...
				$log_naiyou = 'オフィスマスタの更新に失敗しました。オフィス[' . $select_office_cd . ']';	//内容
				...
				$log_office_cd = $office_cd;	//オフィスコード
				$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
				$log_naiyou = 'オフィスマスタを更新しました。オフィス[' . $select_office_cd . ']';	//内容
				$log_err_inf = $query;			//エラー情報
				require( './zs_log.php' );
				//************
				...
				$log_office_cd = $office_cd;	//オフィスコード
				$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
				$log_naiyou = 'オフィスを更新しました。オフィス[' . $select_office_cd . ']';	//内容
				$log_err_inf = '';			//エラー情報
				require( './zs_log.php' );
				//************
				...
				print('<tr>');
				print('<td width="815" align="center"><font color="blue">※更新しました。</font></td>');
