...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kanri_eigyojkn_del1.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kanri_eigyojkn_ksn0.php');
	...
	//エラーなし
		...
		if( $err_flg == 0 ){
			...
			if( $err_cnt == 0 ){
				...
				print('<font color="blue">※以下の期間の営業時間を削除します。よろしいですか？</font><br>');
				print('<font color="red">（まだ削除されていません。）</font><br>');
	
				//有効期間
//				print('<b>有効期間(*)</b>・・・営業時間の有効期間<br>');
				print('<table border="0">');
				print('<tr>');
				print('<td align="left">');
				print('開始日<br>');
				...
				print('<form method="post" action="kanri_eigyojkn_del2.php">');
				print('<td align="right">');
				print('<input type="hidden" name="prc_gmn" value="' . $gmn_id . '">');
				print('<input type="hidden" name="lang_cd" value="' . $lang_cd . '" />');
				print('<input type="hidden" name="office_cd" value="' . $office_cd . '" />');
				...
				print('<font color="red">※有効期間内に予約データが存在するため削除できません。</font><br>');
