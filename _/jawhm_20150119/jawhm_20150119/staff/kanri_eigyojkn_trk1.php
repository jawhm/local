...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kanri_eigyojkn_trk1.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kanri_eigyojkn_trk0.php','kanri_eigyojkn_trk1.php');
	...
	//エラーなし
		...
		if( $err_cnt == 0 ){
			...
			$query = 'select distinct(ST_DATE),ED_DATE from M_EIGYOJKN where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $select_office_cd . '" order by ST_DATE;';
		...
		if( $err_flg == 0 ){
			...
			//明細データにエラーがあるか？
			if( $err_cnt == 0 && $mdata_ttl_err_cnt == 0 ){
				//エラーなし
				...
				print('<form method="post" action="kanri_eigyojkn_trk0.php">');
				...
				print('</td>');
				print('<td align="right">');
				print('<form method="post" action="kanri_eigyojkn_trk2.php">');
				print('<input type="hidden" name="prc_gmn" value="' . $gmn_id . '">');
				print('<input type="hidden" name="lang_cd" value="' . $lang_cd . '" />');
				print('<input type="hidden" name="office_cd" value="' . $office_cd . '" />');
				print('<input type="hidden" name="staff_cd" value="' . $staff_cd . '">');
				...
				print('<table border="0">');
				print('<tr>');
				print('<form name="form5" method="post" action="kanri_eigyojkn_trk1.php">');
				print('<input type="hidden" name="prc_gmn" value="' . $gmn_id . '">');
				print('<input type="hidden" name="lang_cd" value="' . $lang_cd . '" />');
				print('<input type="hidden" name="office_cd" value="' . $office_cd . '" />');
				print('<input type="hidden" name="staff_cd" value="' . $staff_cd . '">');
