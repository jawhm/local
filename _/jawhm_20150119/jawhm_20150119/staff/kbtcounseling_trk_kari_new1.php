...
<title>個別カウンセリング（仮登録からの新規会員登録:確認）</title>
...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kbtcounseling_trk_kari_new1.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kbtcounseling_trk_kari_new0.php','kbtcounseling_trk_kari_new1.php');
...
//				print('</table>');
				...
				print('<form method="post" action="kbtcounseling_trk_kari_new2.php">');
				print('<input type="hidden" name="prc_gmn" value="' . $gmn_id . '">');
				print('<input type="hidden" name="lang_cd" value="' . $lang_cd . '">');
				print('<input type="hidden" name="office_cd" value="' . $office_cd . '">');
				print('<input type="hidden" name="staff_cd" value="' . $staff_cd . '">');
