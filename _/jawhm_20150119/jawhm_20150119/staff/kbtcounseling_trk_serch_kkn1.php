...
<title>個別カウンセリング（会員検索予約登録）</title>
...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kbtcounseling_trk_serch_kkn1.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kbtcounseling_trk_serch_kkn0.php','kbtcounseling_trk_serch_kkn1.php');
	...
	//エラーなし
		...
		print('<img src="./img_' . $lang_cd . '/btn_jkn_saisentaku_2.png" width="0" height="0" style="visibility:hidden;">');
		...
		if( $select_yyk_no != "" ){
			...
			$query = 'select YYK_NO from D_CLASS_YYK where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $select_office_cd . '" and CLASS_CD = "KBT" and YMD = "' . $select_ymd . '" and JKN_KBN = "' . $select_jknkbn . '" and KAIIN_ID = "' . $select_kaiin_no . '";';
				...
				if( $select_yyk_no == "" ){
					...
					if (!$result) {
						...
						if (!$result) {
							...
							$query = 'insert into D_CLASS_YYK values("' . $DEF_kg_cd . '","' . $select_office_cd . '","KBT","' . $select_ymd . '","' . $select_jknkbn . '",' . $yyk_no . ',' . $data_kaiin_kbn[0] . ',';
							$query .= '"' . $select_kaiin_no . '",ENCODE("' . $data_kaiin_nm[0] . '","' . $ANGpw . '"),';
...
//									//************
					...
					//クラス予約（個別カウンセリング）を登録する
					$query = 'insert into D_CLASS_YYK values("' . $DEF_kg_cd . '","' . $select_office_cd . '","KBT","' . $select_ymd . '","' . $select_jknkbn . '",' . $select_yyk_no . ',' . $data_kaiin_kbn[0] . ',';
					$query .= '"' . $select_kaiin_no . '",ENCODE("' . $data_kaiin_nm[0] . '","' . $ANGpw . '"),';
					if( $data_kaiin_kbn[0] == 1 && $data_kaiin_mixi[0] != "" ){
						$query .= '"' . $data_kaiin_mixi[0] . '",';
					}else{
					...
					if (!$result) {
						...
						$yoyakumsg = "";
//						if( $zz_yykinfo_kyoumi != "" ){
//							$yoyakumsg .= "興味のある国：" . $zz_yykinfo_kyoumi . "\n";
//						}
//						if( $zz_yykinfo_jiki != "" ){
//							$yoyakumsg .= "出発予定時期：" . $zz_yykinfo_jiki . "\n";
//						}
						if( $zz_yykinfo_soudan != "" ){
						...
						if ($ret['result'] != 'OK')	{
							...
							if( $zz_yykinfo_znz_mail_send_flg != "" ){
								$query .= $zz_yykinfo_znz_mail_send_flg . ',';
							}else{
								$query .= 'NULL,';
							}
							if( $zz_yykinfo_tjt_mail_send_flg != "" ){
								$query .= $zz_yykinfo_tjt_mail_send_flg . ',';
							}else{
								$query .= 'NULL,';
							}
							$query .= '"' . $zz_yykinfo_yyk_time. '","' . $zz_yykinfo_cancel_time . '","' . $now_time . '","' . $zz_yykinfo_yyk_staff_cd . '","' . $staff_cd . '");';
							$result = mysql_query($query);
							if (!$result) {
								//登録失敗
								$err_flg = 4;
								...
								//変更前のクラス予約を削除する
								$query = 'delete from D_CLASS_YYK where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $zz_yykinfo_office_cd . '" and CLASS_CD = "' . $zz_yykinfo_class_cd . '" and YMD = "' . $zz_yykinfo_ymd . '" and JKN_KBN = "' . $zz_yykinfo_jkn_kbn . '" and YYK_NO = ' . $select_yyk_no . ' and KAIIN_ID = "' . $zz_yykinfo_kaiin_id . '";';
								$result = mysql_query($query);
								if (!$result) {
									$err_flg = 4;
									//エラーメッセージ表示
		...
		if( $err_flg == 0 && $err_cnt == 0 && $data_kaiin_mail[0] != "" && ($data_kaiin_kbn[0] == 1 || $data_kaiin_kbn[0] == 9) ){
			...
			if( $select_ymd > $now_yyyymmdd || ( $select_ymd == $now_yyyymmdd && $Mclass_st_time > ($now_hh * 100 + $now_ii) ) ){
				...
				if( $select_yyk_no != "" && $zz_yykinfo_bikou != "" ){
					//予約時のメアドのみとする
					$send_mail_adr[0] = $zz_yykinfo_bikou;
					$send_mail_adr_cnt = 1;
					
				}else{
					//登録されているメアド全部にメール送信する
			...
			if( $err_cnt == 0 ){
				...
				if( $data_cnt == 1 ){
					...
					if( $select_yyk_no == "" ){
						...
						if( $select_staff_cd != "" ){
							$log_naiyou .= ' 指名[' . $select_open_staff_nm . ']';
						}
						$log_naiyou .= '<br>お客様番号[' . $select_kaiin_no . ']';	//内容
						...
						$log_office_cd = $office_cd;	//オフィスコード
						$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
						$log_naiyou = '個別カウンセリングを変更しました。<br>予約No[' . sprintf("%05d",$yyk_no) . '] 会場[' . $Moffice_office_nm . '] 日時[' . substr($select_ymd,0,4) . '-' . sprintf("%02d",substr($select_ymd,4,2)) . '-' . sprintf("%02d",substr($select_ymd,6,2)) . ' ' . sprintf("%02d",intval($Mclass_st_time / 100)) . ':' . sprintf("%02d",($Mclass_st_time % 100)) . '-' . sprintf("%02d",intval($Mclass_ed_time / 100)) . ':' . sprintf("%02d",($Mclass_ed_time % 100)) . ']';
						if( $zz_yykinfo_staff_cd != "" ){
							$log_naiyou .= ' 指名[' . $zz_yykinfo_open_staff_nm . ']';
						}
						$log_naiyou .= '<br>お客様番号[' . $select_kaiin_no . ']';	//内容
						$log_err_inf = '変更前：会場[' . $zz_yykinfo_office_cd . '] 日付[' . $zz_yykinfo_ymd . '] 時間[' . $zz_yykinfo_st_time . '-' . $zz_yykinfo_ed_time . ']<br>';
						$log_err_inf .= '変更後：会場[' . $select_office_cd . '] 日付[' . $select_ymd . '] 時間[' . $Mclass_st_time . '-' . $Mclass_ed_time . ']<br>mb[' . $mobile_kbn . '] ua[' . $agent1 . '] ip[' . $ip_adr . ']';
						require( './zs_log.php' );
						//************
					...
					if( $select_yyk_no == "" ){
						...
						//予約番号
						print('<tr>');
						print('<td width="110" align="center" valign="middle" background="../img_' . $lang_cd . '/bg_mizu_110x20.png"><img src="./img_' . $lang_cd . '/title_mini_yykno.png" border="0"></td>');
						print('<td width="565" align="center" valign="middle" background="../img_' . $lang_cd . '/bg_mizu_565x20.png"><font size="6" color="blue">' . sprintf("%05d",$yyk_no) . '</font></td>');
						print('</tr>');
						//会場
						print('<tr>');
...
//						if( $kaiin_kyoumi != '' ){
//							print('<font color="blue">' . $kaiin_kyoumi . '</font>');
//						}else{
//							print('<img src="./img_' . $lang_cd . '/mitrk.png" border="0">');	//「未登録」
//						}
//						print('</td>');
...
//						if( $kaiin_jiki != '' ){
//							print('<font color="blue">' . $kaiin_jiki . '</font>');
//						}else{
//							print('<img src="./img_' . $lang_cd . '/mitrk.png" border="0">');	//「未登録」
//						}
//						print('</td>');
...
//####################################################						
						...
						print('<td width="300" align="center" valign="middle" background="../img_' . $lang_cd . '/bg_lightgrey_300x20.png"><font size="4" color="gray">変更前</font></td>');
						...
						print('<td width="300" align="center" valign="middle" background="../img_' . $lang_cd . '/bg_lightgrey_300x20.png"><font size="4" color="gray">' . $zz_yykinfo_office_nm . '</font>');
						if( $zz_yykinfo_staff_cd != "" ){
							//カウンセラー編集
							print('&nbsp;<font size="2" color="gray">(&nbsp;' . $zz_yykinfo_open_staff_nm . '&nbsp;)</font>');
						}
						print('</td>');
						print('</tr>');
						...
						print('<td colspan="3" align="center" valign="middle" bgcolor="#d3d3d3"><font size="6" color="gray">' . substr($zz_yykinfo_ymd,0,4) . '</font><font size="2" color="gray">&nbsp;年&nbsp;</font><font size="6" color="gray">' . sprintf("%d",substr($zz_yykinfo_ymd,5,2)) . '</font><font size="2" color="gray">&nbsp;月&nbsp;</font><font size="6" color="gray">' . sprintf("%d",substr($zz_yykinfo_ymd,8,2))  . '</font><font size="2" color="gray">&nbsp;日&nbsp;<font size="5" color="gray">' . $week[$zz_yykinfo_youbi_cd] . '</font><font size="1" color="gray">&nbsp;曜日</font>&nbsp;&nbsp;&nbsp;<font size="6" color="gray">' .  intval( $zz_yykinfo_st_time / 100 ) . ':' . sprintf("%02d",( $zz_yykinfo_st_time % 100 )) . '&nbsp;-&nbsp;' . intval( $zz_yykinfo_ed_time / 100 ) . ':' . sprintf("%02d",( $zz_yykinfo_ed_time % 100 )) . '</font></td>');
						print('</tr>');
			
						print('</table>');

						//「変更後の日時」
						print('<img src="./img_' . $lang_cd . '/title_henkougo_nichiji_r.png" border="0"><br>');
						
						//背景色
						if( $select_ymd > $now_yyyymmdd2 || ($select_ymd == $now_yyyymmdd2 && $Mclass_st_time > ($now_hh * 100 + $now_ii) )){
							//未来
							$bgfile_1 = "mizu";
							$bgcolor_1 = "#d6fafa";
						}else{
						...
						print('<td width="300" align="center" valign="middle" background="../img_' . $lang_cd . '/bg_' . $bgfile_1 . '_300x20.png"><font size="4" color="blue">' . $Moffice_office_nm . '</font>');
						if( $select_staff_cd != "" ){
							//カウンセラー編集
							print('&nbsp;<font size="2" color="blue">(&nbsp;' . $select_open_staff_nm . '&nbsp;)</font>');
						}
						print('</td>');
						print('</tr>');
						...
						print('<td colspan="3" align="center" valign="bottom" bgcolor="' . $bgcolor_1 . '"><font size="6" color="' . $fontcolor_2 . '">' . substr($select_ymd,0,4) . '</font><font size="1" color="' . $fontcolor_2 . '">&nbsp;年</font>&nbsp;<font size="6" color="' . $fontcolor_2 . '">' . sprintf("%d",substr($select_ymd,4,2)) . '</font><font size="1" color="' . $fontcolor_2 . '">&nbsp;月</font>&nbsp;<font size="6" color="' . $fontcolor_2 . '">' . sprintf("%d",substr($select_ymd,6,2)) . '</font><font size="1" color="' . $fontcolor_2 . '">&nbsp;日</font>&nbsp;<font size="5" color="' . $fontcolor_2 . '">' . $week[$select_youbi_cd] . '</font><font size="1" color="' . $fontcolor_2 . '">&nbsp;曜日</font>&nbsp;<font size="6">' . intval($Mclass_st_time / 100) . ':' . sprintf("%02d",($Mclass_st_time % 100)) . '&nbsp;-&nbsp;' . intval($Mclass_ed_time / 100) . ':' . sprintf("%02d",($Mclass_ed_time % 100)) . '</font></td>');
						print('</tr>');
						...
						if( $zz_yykinfo_kaiin_kbn != "" && $zz_yykinfo_kaiin_kbn == 0 ){
							...
							//メンバー
							print('<font size="5" color="blue">' . $zz_yykinfo_kaiin_id . '</font><br><font size="2">(' . $data_kaiin_mixi[0] . ')</font>');
						}else if( $zz_yykinfo_kaiin_kbn == 9 ){
							//一般（無料メンバー）
					...
					$log_office_cd = $office_cd;	//オフィスコード
					$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
					$log_naiyou = '処理途中で会員・メンバーが削除されたため、個別カウンセリング予約ができませんでした。';	//内容
					$log_err_inf = '';			//エラー情報
					require( './zs_log.php' );
					//************
			...
			$tmp_zz_yykinfo_ymd = substr($zz_yykinfo_ymd,0,4) . substr($zz_yykinfo_ymd,5,2) . substr($zz_yykinfo_ymd,8,2);
			...
			if( $select_yyk_no == "" ){
				...
				print('<tr>');
				print('<td width="110" align="center" valign="middle" background="../img_' . $lang_cd . '/bg_mizu_110x20.png"><img src="./img_' . $lang_cd . '/title_mini_yykno.png" border="0"></td>');
				print('<td width="565" align="center" valign="middle" background="../img_' . $lang_cd . '/bg_mizu_565x20.png"><font size="6" color="blue">' . sprintf("%05d",$tmp_nijyu_yyk_no) . '</font></td>');
				print('</tr>');
				//会場
				print('<tr>');
				print('<td width="110" align="center" valign="middle" background="../img_' . $lang_cd . '/bg_mizu_110x20.png"><img src="./img_' . $lang_cd . '/title_mini_kaijyou.png" border="0"></td>');
				print('<td width="565" align="center" valign="middle" background="../img_' . $lang_cd . '/bg_mizu_565x20.png"><font size="5">' . $zz_yykinfo_office_nm . '</font></td>');
				print('</tr>');
				//日時
				print('<tr>');
				print('<td align="center" valign="middle" background="../img_' . $lang_cd . '/bg_kimidori_110x20.png"><img src="./img_' . $lang_cd . '/title_mini_nichiji.png" border="0"></td>');
				...
				print('<td align="center" valign="bottom" background="../img_' . $lang_cd . '/bg_kimidori_565x20.png">&nbsp;&nbsp;<font size="6" color="' . $fontcolor . '">' . substr($zz_yykinfo_ymd,0,4) . '</font><font size="1" color="' . $fontcolor . '">&nbsp;年</font>&nbsp;<font size="6" color="' . $fontcolor . '">&nbsp;' . sprintf("%d",substr($zz_yykinfo_ymd,5,2)) . '</font><font size="1" color="' . $fontcolor . '">&nbsp;月</font>&nbsp;<font size="6" color="' . $fontcolor . '">&nbsp;' . sprintf("%d",substr($zz_yykinfo_ymd,8,2)) . '</font><font size="1" color="' . $fontcolor . '">&nbsp;日</font>&nbsp;<font size="5" color="' . $fontcolor . '">' . $week[$zz_yykinfo_youbi_cd] . '</font><font size="1" color="' . $fontcolor . '">&nbsp;曜日</font>&nbsp;<font size="6">' . intval($zz_yykinfo_st_time / 100) . ':' . sprintf("%02d",($zz_yykinfo_st_time % 100)) . '&nbsp;-&nbsp;' . intval($zz_yykinfo_ed_time / 100) . ':' . sprintf("%02d",($zz_yykinfo_ed_time % 100)) . '</font></td>');
				print('</tr>');
				//カウンセラー
				print('<tr>');
				print('<td align="center" valign="middle" background="../img_' . $lang_cd . '/bg_pink_110x20.png"><img src="./img_' . $lang_cd . '/title_mini_counselor.png" border="0"></td>');
				...
				if( $zz_yykinfo_eigyoubi_flg == 8 || $zz_yykinfo_eigyoubi_flg == 9 ){
					//非営業日
					print('<font color="red">非営業日です。</font>');
				}else if( $tmp_zz_yykinfo_ymd < $now_yyyymmdd ){
					//「過去日を選択しています」
					print('<img src="./img_' . $lang_cd . '/warning_kakobi.png" border="0">');
				}else if( $tmp_zz_yykinfo_ymd == $now_yyyymmdd && $zz_yykinfo_ed_time <= ($now_hh * 100 + $now_ii) ){
					//「過去時刻を選択しています」
					print('<img src="./img_' . $lang_cd . '/warning_kakojikoku.png" border="0">');
				}
				...
				print('<input type="hidden" name="staff_cd" value="' . $staff_cd . '">');
				print('<input type="hidden" name="select_office_cd" value="' . $select_office_cd . '">');
				print('<input type="hidden" name="select_yyyy" value="">');
				print('<input type="hidden" name="select_mm" value="">');
				print('<input type="hidden" name="select_staff_cd" value="' . $select_staff_cd . '">');
				...
				print('<table border="0">');	//sub2
				print('<tr>');	//sub2
				print('<td width="630" align="left" valign="top">');	//sub2
				...
				print('<td width="150" align="left" valign="top">');
				print('<img src="./img_' . $lang_cd . '/title_okyakusamano.png" border="0"><br>');
				print('<font size="4" color="blue">' . $zz_yykinfo_kaiin_id . '</font>');
				...
				print('<font size="5" color="blue">' . $zz_yykinfo_kaiin_nm . '</font>');
				print('</td>');
				print('</tr>');	
				print('</table>');
				...
				print('</tr>');	//sub2
				print('<tr>');	//sub2
				print('<td align="left" valign="top">');	//sub2
...
//				print('</table>');
			...
			if( $select_yyk_no == "" ){
				...
				//予約番号
				print('<tr>');
				print('<td width="110" height="40" align="center" valign="middle" background="../img_' . $lang_cd . '/bg_lightgrey_110x20.png"><img src="./img_' . $lang_cd . '/title_mini_yykno.png" border="0"></td>');
				//「満席になってしまったので、予約ができませんでした。」
				print('<td width="565" height="40" align="center" valign="middle" background="../img_' . $lang_cd . '/bg_lightgrey_565x20.png"><img src="./img_' . $lang_cd . '/title_ng_manseki.png" border="0"></td>');
				print('</tr>');
				//会場
				print('<tr>');
				print('<td width="110" align="center" valign="middle" background="../img_' . $lang_cd . '/bg_lightgrey_110x20.png"><img src="./img_' . $lang_cd . '/title_mini_kaijyou.png" border="0"></td>');
				print('<td width="565" align="center" valign="middle" background="../img_' . $lang_cd . '/bg_lightgrey_565x20.png"><font size="5" color="gray">' . $Moffice_office_nm . '</font></td>');
				print('</tr>');
				//日時
				print('<tr>');
				print('<td align="center" valign="middle" background="../img_' . $lang_cd . '/bg_lightgrey_110x20.png"><img src="./img_' . $lang_cd . '/title_mini_nichiji.png" border="0"></td>');
				$fontcolor = "gray";
				print('<td align="center" valign="bottom" background="../img_' . $lang_cd . '/bg_lightgrey_565x20.png">&nbsp;&nbsp;<font size="6" color="' . $fontcolor . '">' . substr($select_ymd,0,4) . '</font><font size="1" color="' . $fontcolor . '">&nbsp;年</font>&nbsp;<font size="6" color="' . $fontcolor . '">&nbsp;' . sprintf("%d",substr($select_ymd,4,2)) . '</font><font size="1" color="' . $fontcolor . '">&nbsp;月</font>&nbsp;<font size="6" color="' . $fontcolor . '">&nbsp;' . sprintf("%d",substr($select_ymd,6,2)) . '</font><font size="1" color="' . $fontcolor . '">&nbsp;日</font>&nbsp;<font size="5" color="' . $fontcolor . '">' . $week[$select_youbi_cd] . '</font><font size="1" color="' . $fontcolor . '">&nbsp;曜日</font>&nbsp;<font size="6" color="gray">' . intval($Mclass_st_time / 100) . ':' . sprintf("%02d",($Mclass_st_time % 100)) . '&nbsp;-&nbsp;' . intval($Mclass_ed_time / 100) . ':' . sprintf("%02d",($Mclass_ed_time % 100)) . '</font></td>');
				print('</tr>');
				//カウンセラー
				print('<tr>');
				print('<td align="center" valign="middle" background="../img_' . $lang_cd . '/bg_pink_110x20.png"><img src="./img_' . $lang_cd . '/title_mini_counselor.png" border="0"></td>');
				...
				//「満席になってしまったので、予約ができませんでした。」
				print('<br><img src="./img_' . $lang_cd . '/title_ng_manseki.png" border="0"><br>');
				
				print('<table border="1">');
				print('<tr>');
				print('<td width="110" align="center" valign="middle" background="../img_' . $lang_cd . '/bg_lightgrey_110x20.png"><img src="./img_' . $lang_cd . '/title_mini_yykno.png" border="0"></td>');
				print('<td width="300" align="center" valign="middle" background="../img_' . $lang_cd . '/bg_lightgrey_300x20.png"><font color="gray">変更できません</font></td>');
				print('<td width="110" align="center" valign="middle" background="../img_' . $lang_cd . '/bg_lightgrey_110x20.png"><img src="./img_' . $lang_cd . '/title_mini_kaijyou.png" border="0"></td>');
				print('<td width="300" align="center" valign="middle" background="../img_' . $lang_cd . '/bg_lightgrey_300x20.png"><font size="4" color="gray">' . $Moffice_office_nm . '</font></td>');
				print('</tr>');
				print('<tr>');
				print('<td align="center" valign="middle" background="../img_' . $lang_cd . '/bg_lightgrey_110x20.png"><img src="./img_' . $lang_cd . '/title_mini_nichiji.png" border="0"></td>');
				$fontcolor = "gray";
				print('<td colspan="3" align="center" valign="bottom" bgcolor="#d3d3d3"><font size="6" color="' . $fontcolor . '">' . substr($select_ymd,0,4) . '</font><font size="1" color="' . $fontcolor . '">&nbsp;年</font>&nbsp;<font size="6" color="' . $fontcolor . '">' . sprintf("%d",substr($select_ymd,4,2)) . '</font><font size="1" color="' . $fontcolor . '">&nbsp;月</font>&nbsp;<font size="6" color="' . $fontcolor . '">' . sprintf("%d",substr($select_ymd,6,2)) . '</font><font size="1" color="' . $fontcolor . '">&nbsp;日</font>&nbsp;<font size="5" color="' . $fontcolor . '">' . $week[$select_youbi_cd] . '</font><font size="1" color="' . $fontcolor . '">&nbsp;曜日</font>&nbsp;<font size="6" color="' . $fontcolor . '">' . intval($Mclass_st_time / 100) . ':' . sprintf("%02d",($Mclass_st_time % 100)) . '&nbsp;-&nbsp;' . intval($Mclass_ed_time / 100) . ':' . sprintf("%02d",($Mclass_ed_time % 100)) . '</font></td>');
				print('</tr>');
				print('</table>');
			...
			$tabindex++;
			print('<input type="image" tabindex="' . $tabindex . '" src="./img_' . $lang_cd . '/btn_jkn_saisentaku_1.png" onmouseover="this.src=\'./img_' . $lang_cd . '/btn_jkn_saisentaku_2.png\';" onmouseout="this.src=\'./img_' . $lang_cd . '/btn_jkn_saisentaku_1.png\';" onClick="kurukuru()" border="0">');
			print('</td>');
			print('</form>');
			print('</tr>');
			print('</table>');
