...
<title>スケジュール－個別カウンセリング受付時間帯登録</title>
...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'sc_kbt_trk_top.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('sc_top.php','sc_kbt_trk_selectdate.php');
	...
	//エラーなし
		...
		print('<tr>');
		print('<td width="815" align="center"><font color="blue">※対象のオフィスを指定してください。</font></td>');
		...
		//オフィスマスタの取得
		$Moffice_cnt = 0;
		$query = 'select OFFICE_CD,OFFICE_NM from M_OFFICE where KG_CD = "' . $DEF_kg_cd . '" and YUKOU_FLG = 1 order by ST_DATE,OFFICE_CD;';
		...
		if( $err_flg == 0 ){
			...
			while( $i < $Moffice_cnt ){
				...
				print('>' . $Moffice_office_cd[$i] . '：' . $Moffice_office_nm[$i] . '&nbsp;&nbsp;&nbsp;</option>');
				$i++;
			}
			print('</select></td>');
			print('</tr></table>');
