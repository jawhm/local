...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kbtcounseling_trk_kari_serch_kkn0.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kbtcounseling_trk_kari_serch.php','kbtcounseling_trk_kari_serch_kkn0.php');
	...
	//エラーなし
		...
		//固有引数の取得
		if( $prc_gmn == "kbtcounseling_trk_kari_serch_kkn1.php" ){
		...
		//オフィスマスタの取得
		$query = 'select OFFICE_NM,START_YOUBI from M_OFFICE where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $zz_yykinfo_office_cd . '";';
		...
		$Meigyojkn_cnt = 0;
		$query = 'select YOUBI_CD,TEIKYUBI_FLG,OFFICE_ST_TIME,OFFICE_ED_TIME,ST_DATE,ED_DATE from M_EIGYOJKN where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $zz_yykinfo_office_cd . '" and YUKOU_FLG = 1 and ED_DATE >= "' . $zz_yykinfo_ymd . '" order by YOUBI_CD,ST_DATE;';
		$result = mysql_query($query);
		if (!$result) {
			$err_flg = 4;
			//エラーメッセージ表示
		...
		if( $err_flg == 0 ){
			...
			$query = 'select EIGYOUBI_FLG from M_CALENDAR where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $zz_yykinfo_office_cd . '" and YMD  = "' . $zz_yykinfo_ymd . '";';
			$result = mysql_query($query);
			if (!$result) {
				$err_flg = 4;
				//エラーメッセージ表示
		...
		if( $select_teikyubi_flg == 1 ){
			$query = 'select OFFICE_ST_TIME,OFFICE_ED_TIME from D_EIGYOJKN_KBT where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $zz_yykinfo_office_cd . '" and YMD = "' . $zz_yykinfo_ymd . '";';
			$result = mysql_query($query);
			if (!$result) {
				$err_flg = 4;
				//エラーメッセージ表示
		...
		$Mclass_st_time = 0;
		$Mclass_ed_time = 0;
		$query = 'select ST_TIME,ED_TIME from M_CLASS_JKNWR where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $zz_yykinfo_office_cd . '" and CLASS_CD = "KBT" and JKN_KBN = "' . $zz_yykinfo_jkn_kbn . '" and ST_DATE <= "' . $zz_yykinfo_ymd . '" and ED_DATE >= "' . $zz_yykinfo_ymd . '" and YUKOU_FLG = 1;';
		$result = mysql_query($query);
		if (!$result) {
			$err_flg = 4;
			//エラーメッセージ表示
		...
		if( $err_flg == 0 ){
			if( $err_cnt == 0 ){
				...
				if( $data_cnt == 1 ){
					...
					print('<form method="post" action="kbtcounseling_trk_kari_serch_kkn1.php">');
					print('<input type="hidden" name="prc_gmn" value="' . $gmn_id . '">');
					print('<input type="hidden" name="lang_cd" value="' . $lang_cd . '" />');
					print('<input type="hidden" name="office_cd" value="' . $office_cd . '" />');
					print('<input type="hidden" name="staff_cd" value="' . $staff_cd . '">');
					...
					while ($i < $data_cnt && $i < $max_cnt ){
						...
						print('<tr>');
						print('<form method="post" action="kbtcounseling_trk_kari_serch_kkn0.php">');
						print('<input type="hidden" name="prc_gmn" value="' . $gmn_id . '">');
						print('<input type="hidden" name="lang_cd" value="' . $lang_cd . '" />');
						print('<input type="hidden" name="office_cd" value="' . $office_cd . '" />');
						print('<input type="hidden" name="staff_cd" value="' . $staff_cd . '">');
					...
					print('<table border="0">');
					print('<tr>');
					print('<form method="post" action="kbtcounseling_trk_kari_new0.php">');
					print('<input type="hidden" name="prc_gmn" value="' . $gmn_id . '">');
					print('<input type="hidden" name="lang_cd" value="' . $lang_cd . '">');
					print('<input type="hidden" name="office_cd" value="' . $office_cd . '">');
					print('<input type="hidden" name="staff_cd" value="' . $staff_cd . '">');
