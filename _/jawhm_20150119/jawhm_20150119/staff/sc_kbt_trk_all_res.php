...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'sc_kbt_trk_all_res.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('sc_kbt_trk_all.php');
	...
	//エラーなし
		...
		print('<img src="./img_' . $lang_cd . '/btn_henkou2_2.png" width="0" height="0" style="visibility:hidden;">');
		...
		if (!$result) {
			...
			$Moffice_st_date = substr($row[2],0,4) . substr($row[2],5,2) . substr($row[2],8,2);		//開始日
			$Moffice_ed_date = substr($row[3],0,4) . substr($row[3],5,2) . substr($row[3],8,2);		//終了日
		}
		...
		//前週
		$zen_week_yyyymmdd = date("Ymd", mktime(0, 0, 0, sprintf("%d",substr($target_yyyymmdd,4,2)), (sprintf("%d",substr($target_yyyymmdd,6,2)) - 7) , sprintf("%d",substr($target_yyyymmdd,0,4))) );
		//翌週
		$yoku_week_yyyymmdd = date("Ymd", mktime(0, 0, 0, sprintf("%d",substr($target_yyyymmdd,4,2)), (sprintf("%d",substr($target_yyyymmdd,6,2)) + 7) , sprintf("%d",substr($target_yyyymmdd,0,4))) );
		...
		if( $err_flg == 0 ){
			...
			print('<option value="" class="color2" selected>全員表示</option>');
			$s = 0;
			while( $s < $Mstaff_cnt ){
				print('<option value="' . $Mstaff_staff_cd[$s] . '" class="color1" >' . $Mstaff_staff_nm[$s] . '</option>');
				$s++;
			}
			print('</select>');
			print('</td>');
			...
			print('<input type="hidden" name="staff_cd" value="' . $staff_cd . '">');
			print('<input type="hidden" name="select_office_cd" value="' . $select_office_cd . '">');
			print('<input type="hidden" name="select_ymd" value="' . $zen_week_yyyymmdd . '">');
			print('<input type="hidden" name="select_staff_cd" value="' . $select_staff_cd . '">');
			print('<td width="135" align="center" valign="middle">');
			if( $zen_week_yyyymmdd <= $Moffice_ed_date ){
				//前週
				$tabindex++;
			...
			print('</td>');
			print('</form>');
			print('<td width="200" align="center" valign="middle">');
			//「登録しました」
			print('<img src="./img_' . $lang_cd . '/title_trk_ok.png">');
			print('</td>');
			print('<form method="post" action="sc_kbt_trk_all.php#' . $target_yyyymmdd . '">');
			print('<input type="hidden" name="prc_gmn" value="' . $gmn_id . '">');
			print('<input type="hidden" name="lang_cd" value="' . $lang_cd . '" />');
			...
			print('<input type="hidden" name="staff_cd" value="' . $staff_cd . '">');
			print('<input type="hidden" name="select_office_cd" value="' . $select_office_cd . '">');
			print('<input type="hidden" name="select_ymd" value="' . $yoku_week_yyyymmdd . '">');
			print('<input type="hidden" name="select_staff_cd" value="' . $select_staff_cd . '">');
			print('<td width="135" align="center" valign="middle">');
			if( $yoku_week_yyyymmdd <= $Moffice_ed_date ){
				//翌週
				$tabindex++;
			...
			$log_office_cd = $office_cd;	//オフィスコード
			$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
			$log_naiyou = '個別カウンセリングのスケジュールを登録しました。<br>対象オフィス[' . $select_office_cd . '] 選択日[' . $select_ymd . ']';	//内容
			$log_err_inf = '対象オフィス[' . $select_office_cd . '] 選択日[' . $select_ymd . '] 指定スタッフ[' . $select_staff_cd . ']';			//エラー情報
			require( './zs_log.php' );
			//************
			...
			while( $t < 7 ){
				...
				$name = 'edit_ymd_' . $t;
				$edit_ymd = $_POST[$name];
				$name = 'edit_staff_cnt_' . $t;
				$edit_staff_cnt = $_POST[$name];
				...
				//カウンセラー（スタッフ）
				$s = 0;
				while( $s < $edit_staff_cnt ){
					//編集画面で表示したカウンセラー分 繰り返す
					
					//*** 引数取得 ***
					//スタッフコード
					$name = 'edit_staff_cd_' . $t . '_' . $s;
					$edit_staff_cd = $_POST[$name];
					//公開フラグ
					$name = 'edit_open_flg_' . $t . '_' . $s;
					$edit_open_flg = $_POST[$name];
					//一括登録（受付フラグ）
					$name = 'edit_ikt_flg_' . $t . '_' . $s;
					$edit_ikt_flg = $_POST[$name];
					
					//スタッフ名を求める
					$edit_staff_nm = '<font color="red">取得エラー</font>';
					$z = 0;
					$find_flg = 0;
					while( $z < $Mstaff_cnt && $find_flg == 0 ){
						if( $edit_staff_cd == $Mstaff_staff_cd[$z] ){
							$edit_staff_nm = $Mstaff_staff_nm[$z];
							$find_flg = 1;
						}
						$z++;	
					}
					if( $find_flg == 0 ){
						...
						$log_office_cd = $office_cd;	//オフィスコード
						$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
						$log_naiyou = '引数：スタッフコードが見つからない。検索対象のスタッフコード[' . $edit_staff_cd . ']';	//内容
						$log_err_inf = "";			//エラー情報
						require( './zs_log.php' );
						//************
						...
						//スタッフ名
						print('<td width="180" colspan="2" align="left" valign="middle" background="../img_' . $lang_cd . '/' . $bgfile . '_180x20.png"><b>&nbsp;&nbsp;' . $edit_staff_nm . '</b></td>');
						...
						while( $j < $Mclassjknwr_cnt ){
							...
							//*** 引数取得 ***
							//受付フラグ
							$name = 'edit_uktk_flg_' . $t . '_' . $s . '_' . $Mclassjknwr_jkn_kbn[$j];
							$edit_uktk_flg = $_POST[$name];
							if( $edit_uktk_flg == "" ){
								$edit_uktk_flg = 0;		//0:未登録 1:受付可 2:受付不可
							}
							if( $edit_uktk_flg == 0 && $edit_ikt_flg != 0 ){
								//一括登録に設定があり、該当時間帯が未設定の場合は、一括登録で指定した受付区分で上書きする
								$edit_uktk_flg = $edit_ikt_flg;
							}
							
							//スタッフスケジュールへ登録する
							$err_flg = 0;
							$query = 'select count(*) from D_STAFF_SC where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $select_office_cd . '" and YMD = "' . $target_yyyymmdd . '" and STAFF_CD = "' . $edit_staff_cd . '" and CLASS_CD = "KBT" and JKN_KBN = "' . $Mclassjknwr_jkn_kbn[$j] . '";';
							$result = mysql_query($query);
							if (!$result) {
								$err_flg = 1;
								...
								if( $row[0] == 0 ){
									...
									$query = 'insert into D_STAFF_SC values("' . $DEF_kg_cd . '","' . $select_office_cd . '","' . $target_yyyymmdd . '","' . $edit_staff_cd . '","KBT","' . $Mclassjknwr_jkn_kbn[$j] . '","' . $edit_open_flg . '","' . $edit_uktk_flg . '","' . $now_time . '","' . $staff_cd . '");';
									$result = mysql_query($query);
									if (!$result) {
										$err_flg = 1;
										...
										$log_office_cd = $office_cd;	//オフィスコード
										$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
										$log_naiyou = 'スタッフスケジュールの登録に失敗しました。';	//内容
										$log_err_inf = $query;			//エラー情報
										require( './zs_log.php' );
										//************
										...
										$log_office_cd = $office_cd;	//オフィスコード
										$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
										$log_naiyou = 'スタッフスケジュールを登録しました。';	//内容
										$log_err_inf = $query;			//エラー情報
										require( './zs_log.php' );
										//************
									...
									//更新
									require( '../zz_mojicd.php' );
									$query = 'update D_STAFF_SC set OPEN_FLG = "' . $edit_open_flg . '",UKTK_FLG = "' . $edit_uktk_flg . '",UPDATE_TIME = "' . $now_time . '",UPDATE_STAFF_CD = "' . $staff_cd . '" where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $select_office_cd . '" and YMD = "' . $target_yyyymmdd . '" and STAFF_CD = "' . $edit_staff_cd . '" and CLASS_CD = "KBT" and JKN_KBN = "' . $Mclassjknwr_jkn_kbn[$j] . '";';
									$result = mysql_query($query);
									if (!$result) {
										$err_flg = 1;
										...
										$log_office_cd = $office_cd;	//オフィスコード
										$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
										$log_naiyou = 'スタッフスケジュールの更新に失敗しました。';	//内容
										$log_err_inf = $query;			//エラー情報
										require( './zs_log.php' );
										//************
										...
										$log_office_cd = $office_cd;	//オフィスコード
										$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
										$log_naiyou = 'スタッフスケジュールを更新しました。';	//内容
										$log_err_inf = $query;			//エラー情報
										require( './zs_log.php' );
										//************
							...
							$tmp_shimei_flg = 0;
							$query = 'select count(*) from D_CLASS_YYK where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $select_office_cd . '" and CLASS_CD = "KBT" and YMD = "' . $target_yyyymmdd . '" and JKN_KBN = "' . $Mclassjknwr_jkn_kbn[$j] . '" and STAFF_CD = "' . $edit_staff_cd . '";';
							$result = mysql_query($query);
							if (!$result) {
								$err_flg = 1;
							...
							if( $err_flg != 0 ){
								...
								//営業時間外
								print('background="../img_' . $lang_cd . '/bg_lightgrey_80x20.png"');
							}else if( $edit_open_flg == 0 ){
								//非公開
								print('background="../img_' . $lang_cd . '/bg_yellow_80x20.png"');
							}else if( $tmp_shimei_flg == 1 ){
								//指名予約
								print('background="../img_' . $lang_cd . '/bg_blue_80x20.png"');
							}else if( $edit_uktk_flg == 2 ){
								//受付不可
								print('background="../img_' . $lang_cd . '/bg_pink_80x20.png"');
							}else if( $edit_uktk_flg == 0 ){
								//未設定
								print('background="../img_' . $lang_cd . '/bg_yellow_80x20.png"');
							}else if( in_array($Mclassjknwr_jkn_kbn[$j] , $jkn_kbn_array_1) ){
								//メンバー および 一般 の時間区分
							...
							if( $err_flg != 0 ){
								//エラーあり
								print('<img src="./img_' . $lang_cd . '/title_uktk_err.png">');
							}else if( $tmp_shimei_flg == 1 ){
								// ☆ 指名予約
								print('<img src="./img_' . $lang_cd . '/title_uktk_select_staff.png">');
							}else if( $edit_uktk_flg == 1 ){
								// Ｏ 受付可
			...
			print('<form method="post" action="sc_kbt_trk_all.php#' . $select_ymd . '"">');
			print('<input type="hidden" name="prc_gmn" value="' . $gmn_id . '">');
			print('<input type="hidden" name="lang_cd" value="' . $lang_cd . '" />');
			print('<input type="hidden" name="office_cd" value="' . $office_cd . '" />');
			print('<input type="hidden" name="staff_cd" value="' . $staff_cd . '">');
			...
			print('<td align="right">');
			$tabindex++;
			print('<input type="image" tabindex="' . $tabindex . '" src="./img_' . $lang_cd . '/btn_henkou2_1.png" onmouseover="this.src=\'./img_' . $lang_cd . '/btn_henkou2_2.png\';" onmouseout="this.src=\'./img_' . $lang_cd . '/btn_henkou2_1.png\';" onClick="kurukuru()" border="0">');
			print('</td>');
			print('</form>');
			print('</tr>');
			print('<tr>');
