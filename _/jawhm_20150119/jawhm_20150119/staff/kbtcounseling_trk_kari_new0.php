...
<title>個別カウンセリング（仮登録からの新規会員登録）</title>
...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kbtcounseling_trk_kari_new0.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kbtcounseling_trk_kari_serch.php','kbtcounseling_trk_kari_serch_kkn0.php','kbtcounseling_trk_kari_new1.php');
	...
	//エラーなし
		...
		$kaiin_soudan = '';		//相談内容
		if( $prc_gmn == 'kbtcounseling_trk_kari_new1.php' || $prc_gmn == 'kbtcounseling_trk_kari_new2.php' ){
		...
		if( $err_flg == 0 ){
			...
			print('<form method="post" action="kbtcounseling_trk_kari_new1.php">');
			print('<input type="hidden" name="prc_gmn" value="' . $gmn_id . '">');
			print('<input type="hidden" name="lang_cd" value="' . $lang_cd . '" />');
			print('<input type="hidden" name="office_cd" value="' . $office_cd . '" />');
			print('<input type="hidden" name="staff_cd" value="' . $staff_cd . '">');
...
//			print('</table>');
			...
			print('<td width="725" align="left" valign="top">');
			//記入例
