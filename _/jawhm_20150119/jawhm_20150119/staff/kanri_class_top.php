...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kanri_class_top.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kanri_top.php','kanri_class_select.php','kanri_class_trk0.php','kanri_class_trk1.php','kanri_class_trk2.php',
	...
	//エラーなし
		...
		print('<tr>');
		print('<td width="815" align="center"><font color="blue">※予約種別を設定するオフィスを指定してください。</font></td>');
		...
		if( $err_flg == 0 ){
			...
			print('<form name="form1" method="post" action="kanri_class_select.php">');
			print('<input type="hidden" name="prc_gmn" value="' . $gmn_id . '">');
			print('<input type="hidden" name="lang_cd" value="' . $lang_cd . '" />');
			print('<input type="hidden" name="office_cd" value="' . $office_cd . '" />');
			print('<input type="hidden" name="staff_cd" value="' . $staff_cd . '">');
