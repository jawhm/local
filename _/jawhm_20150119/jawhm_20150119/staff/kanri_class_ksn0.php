...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kanri_class_ksn0.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kanri_class_select.php','kanri_class_ksn1.php','kanri_class_del1.php');
	...
	//エラーなし
		...
		if( $err_flg == 0 ){
			...
			print('<font color="blue">※変更後、登録ボタンを押下してください。（*印の項目は必須入力となります。)</font><br>');
			...
			print('<tr>');
			print('<td width="815" align="left">&nbsp;</td>');
			print('<form method="post" action="kanri_class_del1.php">');
			print('<td align="right">');
			print('<input type="hidden" name="prc_gmn" value="' . $gmn_id . '">');
			print('<input type="hidden" name="lang_cd" value="' . $lang_cd . '" />');
			print('<input type="hidden" name="office_cd" value="' . $office_cd . '" />');
