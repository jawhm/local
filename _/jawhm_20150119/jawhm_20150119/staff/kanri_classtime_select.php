...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kanri_classtime_select.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kanri_top.php','kanri_classtime_top.php','kanri_classtime_select_2.php');
	...
	//エラーなし
		...
		//クラスマスタの取得
		$Mclass_cnt = 0;
		$query = 'select DISTINCT(CLASS_CD),CLASS_NM from M_CLASS where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $select_office_cd . '"  order by CLASS_CD;';
		$result = mysql_query($query);
		if (!$result) {
			$err_flg = 4;
			//エラーメッセージ表示
		...
		if( $err_flg == 0 ){
			...
			if( $trk_office_su == 1 ){
				...
				print('<form method="post" action="kanri_classtime_top.php">');
				print('<input type="hidden" name="prc_gmn" value="' . $gmn_id . '">');
				print('<input type="hidden" name="lang_cd" value="' . $lang_cd . '" />');
				print('<input type="hidden" name="office_cd" value="' . $office_cd . '" />');
				print('<input type="hidden" name="staff_cd" value="' . $staff_cd . '">');
			...
			print('<font color="blue">※予約種別を指定してください。</font><br>');

			print('<form name="form1" method="post" action="kanri_classtime_select_2.php">');
			print('<input type="hidden" name="prc_gmn" value="' . $gmn_id . '">');
			print('<input type="hidden" name="lang_cd" value="' . $lang_cd . '" />');
			print('<input type="hidden" name="office_cd" value="' . $office_cd . '" />');
			print('<input type="hidden" name="staff_cd" value="' . $staff_cd . '">');
			...
			print('<table  border="0">');
			print('<tr>');
			print('<td width="250" align="left"><img src="./img_' . $lang_cd . '/bar_classnm.png"></td>');
			...
			print('<tr>');
			print('<td align="left">');
			print('<select name="select_class_cd" style="font-size:20px;" class="normal">');
			$i = 0;
			while( $i < $Mclass_cnt ){
				print('<option value="' . $Mclass_class_cd[$i] . '">' . $Mclass_class_cd[$i] . '：' . $Mclass_class_nm[$i] . '</option>');
				$i++;
			}
			print('</select>');
			print('</td>');
