...
<title>管理画面－クラス時間割（新規登録）結果</title>
...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kanri_classtime_trk2.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kanri_classtime_trk1.php');
		...
		if( $err_flg == 0 ){
			...
			while( $i < 48 ){
				if( $jkn_kbn[$i] != ''){
					...
					$query = 'insert into M_CLASS_JKNWR values ("' . $DEF_kg_cd . '","' . $select_office_cd . '","' . $select_class_cd . '",' .
						($i + 1 ) . ',"' . $jkn_kbn[$i] . '","' . $st_date . '","' . $ed_date . '",' . $st_time[$i] . ',' . $ed_time[$i] .
						',' . $yukou_flg . ',"' . $now_time . '","' . $staff_cd . '")';
					...
					if (!$result) {
						...
						$log_office_cd = $office_cd;	//オフィスコード
						$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
						$log_naiyou = 'クラス時間割へinsertしました。[' . $select_office_cd . ']-[' . $select_class_cd . ']-[' . $st_date . ']-[' . $jkn_kbn[$i] . ']';	//内容
						$log_err_inf = $query;			//エラー情報
						require( './zs_log.php' );
						//************
			...
			if( $err_cnt == 0 ){
				...
				$log_office_cd = $office_cd;	//オフィスコード
				$log_kaiin_no = $staff_cd;		//会員番号 または スタッフコード
				$log_naiyou = '予約種別時間割を登録しました。<br>オフィス[' . $select_office_cd . '] クラス[' . $select_class_cd . '] 開始日[' . sprintf("%04d",$st_year) . '-' . sprintf("%02d",$st_month) . '-' . sprintf("%02d",$st_day) . ']';	//内容
				$log_err_inf = '';			//エラー情報
				require( './zs_log.php' );
				//************
				...
				print('<font color="blue">※登録しました。</font><br><br>');
				...
				//ページ編集
				print('<table bgcolor="pink"><tr><td width="950">');
				print('<img src="./img_' . $lang_cd . '/bar_kanri_studiojknwr.png" border="0">');
				print('</td></tr></table>');
				...
				print('<font color="red">※登録に失敗しました。</font><br>');
