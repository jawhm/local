...
<?php
	...
	//画面情報
	//当画面の画面ＩＤ
	$gmn_id = 'kanri_eigyojkn_trk0.php';
	//遷移ＯＫとする遷移元の画面ＩＤ
	$ok_gmn = array('kanri_eigyojkn_select.php','kanri_eigyojkn_trk1.php','kanri_eigyojkn_trk2.php');
	...
	//エラーなし
		...
		//営業時間マスタの取得
		$query = 'select max(ED_DATE) from M_EIGYOJKN where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $select_office_cd . '";';
		...
		if (!$result) {
			...
			if( $row[0] != '' ){
				...
				$st_month = sprintf("%d",substr( $next_date,5,2));	//開始月
				$st_day = sprintf("%d",substr( $next_date,8,2));	//開始日
		...
		$teikyu_0 = '';			//定休日チェックボックス（日曜） 'on':チェックあり
		$tnp_st_time_0 = '';	//開店時刻（日曜）
		$tnp_ed_time_0 = '';	//閉店時刻（日曜）
		$teikyu_1 = '';			//定休日チェックボックス（月曜） 'on':チェックあり
		$tnp_st_time_1 = '';	//開店時刻（月曜）
		$tnp_ed_time_1 = '';	//閉店時刻（月曜）
		$teikyu_2 = '';			//定休日チェックボックス（火曜） 'on':チェックあり
		$tnp_st_time_2 = '';	//開店時刻（火曜）
		$tnp_ed_time_2 = '';	//閉店時刻（火曜）
		$teikyu_3 = '';			//定休日チェックボックス（水曜） 'on':チェックあり
		$tnp_st_time_3 = '';	//開店時刻（水曜）
		$tnp_ed_time_3 = '';	//閉店時刻（水曜）
		$teikyu_4 = '';			//定休日チェックボックス（木曜） 'on':チェックあり
		$tnp_st_time_4 = '';	//開店時刻（木曜）
		$tnp_ed_time_4 = '';	//閉店時刻（木曜）
		$teikyu_5 = '';			//定休日チェックボックス（金曜） 'on':チェックあり
		$tnp_st_time_5 = '';	//開店時刻（金曜）
		$tnp_ed_time_5 = '';	//閉店時刻（金曜）
		$teikyu_6 = '';			//定休日チェックボックス（土曜） 'on':チェックあり
		$tnp_st_time_6 = '';	//開店時刻（土曜）
		$tnp_ed_time_6 = '';	//閉店時刻（土曜）
		$teikyu_7 = '';			//定休日チェックボックス（土日祝の前日） 'on':チェックあり
		$tnp_st_time_7 = '';	//開店時刻（土日祝の前日）
		$tnp_ed_time_7 = '';	//閉店時刻（土日祝の前日）
		$teikyu_8 = '';			//定休日チェックボックス（祝日） 'on':チェックあり
		$tnp_st_time_8 = '';	//開店時刻（祝日）
		$tnp_ed_time_8 = '';	//閉店時刻（祝日）
		$yukou_flg = 1;			//有効フラグ　0：無効　1：有効
		$ed_year = 2037;		//終了年
		$ed_month = 12;			//終了月
		$ed_day = 31;			//終了日
		
		if( $prc_gmn == 'kanri_eigyojkn_trk1.php' ){
		...
		if( $err_flg == 0 ){
			...
			print('<table border="0"');
			print('<tr>');
			print('<form name="form2" method="post" action="kanri_eigyojkn_trk1.php">');
			print('<input type="hidden" name="prc_gmn" value="' . $gmn_id . '">');
			print('<input type="hidden" name="lang_cd" value="' . $lang_cd . '" />');
			print('<input type="hidden" name="office_cd" value="' . $office_cd . '" />');
			print('<input type="hidden" name="staff_cd" value="' . $staff_cd . '">');
