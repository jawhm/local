...
<?php
	...
	// IDの取得
	if($err_flg == 0){
		$sql = "SELECT id FROM event_list WHERE hiduke = \"".$kinou."\" AND k_use = 1";
	...
	if($err_flg == 0){
		...
		for($i=0; $i<count($id); $i++){
			$sql = "SELECT email FROM entrylist WHERE seminarid = \"".$id[$i]."\" AND stat = 5";
	...
	if($err_flg == 0){
		...
		while($i<count($email)){
			...
			$sql .= "VALUES(NULL, '30', \"".$email[$i]."\", NULL, NULL, NULL, '7',\"". $date."\", 0, NULL)";
			$result = mysql_query($sql, $link);
	...
	$title = "【ステップ登録】「昨日セミナーキャンセル者パターン登録」";
