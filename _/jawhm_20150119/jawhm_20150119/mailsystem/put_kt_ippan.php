...
<h1 style="color:white;background-color:#525D76;font-size:22px;">協会（日本人）（メール会員）の一括送信</h1>
<?php
...
if($_POST['submit'] != NULL){
	...
	try {
		...
		$rs = $db->query('SELECT vmail as email, vname1 as namae , vcheck FROM maillist WHERE vsend = "1" AND vtype = "kt_ippan"');
		$cnt = 0;
		while($row = $rs->fetch(PDO::FETCH_ASSOC)){
			...
			$headers = array(
                ...
                'From' => mb_encode_mimeheader('日本ワーキングホリデー協会','ISO-2022-JP').'<j-doi@plate.co.jp>',
			...
			$q->put('j-doi@plate.co.jp', $row['email'], $headers, $body);
		}
...
?>
	<form method="POST" action="put_kt_ippan.php" onsubmit="return(confirm('本当に送信しますか？'));">
		件名：<br />
