...
//	for($i=$limit_date['day'];$i<=$limit_date['last_day_to_search'];$i++)
	...
	$month = date('m', strtotime($start_ymd));
	$saisho = date('d', strtotime($start_ymd));
	for($i=$saisho; $i<=$saisho+$sa; $i++)
	{
		...
		if($i==$limit_date['day'])
		{
			...
			$ret .= calender_output($cal, $i,$w,$year,$month,"",$place_name,$keyword,$selected_day,$limit_date['date_limit_beginning'],$limit_date['half_month'],true);
