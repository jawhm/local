...
class SeminarModule
{
	...
	*/
	...
	function __construct($config = array())
	{
		...
		$data_cookie = base64_encode(serialize($this->_config));
	...
	private function _get_seminar_list()
	{
		...
		*/
		...
		foreach ($list as $row) {
			...
			$icon_live = '_bloadcast';
			if ($row['broadcasting'] == 0) {
				$icon_live = '_no_bloadcast';
			}
			$indication = '';
...
';
	...
	private function _show_list($target_seminar_id = "")
	{
		...
		if ($this->_is_list_view) $output .= $this->yoyakuform();
	...
	<div><div class="btn_close_info"><input type="button" class="button_cancel" value=" 取消 " onclick="btn_cancel();" /></div>
		...
		<table style="width:560px; background-color: lightblue;">
			<tr style="">
				...
				<td style="text-align:left;">' . $this->_douji_info[0]['start'] . '〜</td>
			</tr>
			<tr style="">
				<td nowrap style="text-align:right;">セミナー名&nbsp;</td>
				<td style="text-align:left;">' . $this->_douji_info[0]['k_title1'] . '</td>
			</tr>
		</table>
		<br>
		<div style="font-size:9pt; font-weight:bold; margin:10px 0 10px 0;">
			<p>このセミナーは複数の会場で開催されます。<br>どこの会場で予約しますか？</p>
		</div>
		<br>
		...
		foreach ($place_list as $k => $p) {
			...
			for ($i = 0; $i < $diff; $i++) $p .= '　';
			$output .= '<input type="button" class="button_submit" id="" value=" ' . $p . '" onClick="location.href=\'/seminar/seminar?num=' . $k . '&view=1#calendar_start\'"><br><br>';
		}
		$output .= '</div>
	</div>
</div>
...
なお、当日のセミナーのご予約は、03-6304-5858までご連絡ください。<br/>
	...
	private function _check_event()
	{
		if (!empty($this->_num))
		{
			...
			//if ($this->_selected_event_info['place'] == 'event' && !$this->is_mobile()) {
