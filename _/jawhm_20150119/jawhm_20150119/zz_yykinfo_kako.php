<?php
	...
	// $zz_yykinfo_status		: ステータス 0:本人登録 1:スタッフ受付 2:非来店 3:来店 7:日時変更 8:本人キャンセル登録 9:スタッフキャンセル登録
	...
	// $zz_yykinfo_cancel_time_r: キャンセル日時 [yyyy-mm-dd HH:ii:ss]形式
	// $zz_yykinfo_yyk_staff_cd	: 予約受付したスタッフコード
	// $zz_yykinfo_yyk_staff_nm	: 予約受付したスタッフ名
	// $zz_yykinfo_cancel_staff_nm	: 予約取消を行ったスタッフ名
	// $zz_yykinfo_new_ymd		: 現在の年月（予約が無い場合は未設定）
	// $zz_yykinfo_new_jkn_kbn	: 現在の時間区分（予約が無い場合は未設定）
	// $zz_yykinfo_new_st_time	: 現在の開始時刻（予約が無い場合は未設定）
	// $zz_yykinfo_new_ed_time	: 現在の終了時刻（予約が無い場合は未設定）
	//
	//*********************************************************
	if( $zz_yykinfo_yyk_no == "" ){
		...
		$zz_yykinfo_cancel_time_r = "";		//キャンセル日時
		$zz_yykinfo_yyk_staff_cd = "";		//予約受付スタッフコード
		$zz_yykinfo_cancel_staff_cd = "";	//予約取消を行ったスタッフコード
		$zz_yykinfo_youbi_cd = "";			//曜日コード

		$zz_yykinfo_cnt = 0;
		$zz_query = 'select OFFICE_CD,CLASS_CD,YMD,JKN_KBN,KAIIN_KBN,KAIIN_ID,MIXI,STAFF_CD,STATUS,DECODE(KYOUMI,"' . $ANGpw . '"),DECODE(JIKI,"' . $ANGpw . '"),' . 
					' DECODE(SOUDAN,"' . $ANGpw . '"),DECODE(BIKOU,"' . $ANGpw . '"),ZNZ_MAIL_SEND_FLG,TJT_MAIL_SEND_FLG,YYK_TIME,CANCEL_TIME,CANCEL_TIME_R,YYK_STAFF_CD,CANCEL_STAFF_CD ' .
					' from D_CLASS_YYK_CAN where KG_CD = "' . $DEF_kg_cd . '" and YYK_NO = "' . $zz_yykinfo_yyk_no . '" order by CANCEL_TIME_R desc LIMIT 1;';
		...
		if (!$zz_result) {
			...
			while( $zz_row = mysql_fetch_array($zz_result) ){
				...
				$zz_yykinfo_cancel_time_r = $zz_row[17];		//キャンセル日時
				$zz_yykinfo_yyk_staff_cd = $zz_row[18];			//予約受付スタッフコード
				$zz_yykinfo_cancel_staff_cd = $zz_row[19];		//予約取消を行ったスタッフコード
		...
		if( $zz_yykinfo_cnt == 0 ){
			...
			if( $zz_yykinfo_rtncd == 0 && $zz_yykinfo_yyk_staff_cd != "" ){
				$zz_query = 'select DECODE(STAFF_NM,"' . $ANGpw . '") from M_STAFF where KG_CD = "' . $DEF_kg_cd . '" and STAFF_CD = "' . $zz_yykinfo_yyk_staff_cd . '";';
				$zz_result = mysql_query($zz_query);
				if (!$zz_result) {
					$zz_yykinfo_rtncd = 1;	//リターンコード 0:正常 1:エラー
			...
			//予約取消を行ったスタッフ名
			$zz_yykinfo_cancel_staff_nm = "";
			if( $zz_yykinfo_rtncd == 0 && $zz_yykinfo_cancel_staff_cd != "" ){
				$zz_query = 'select DECODE(STAFF_NM,"' . $ANGpw . '") from M_STAFF where KG_CD = "' . $DEF_kg_cd . '" and STAFF_CD = "' . $zz_yykinfo_cancel_staff_cd . '";';
				$zz_result = mysql_query($zz_query);
				if (!$zz_result) {
					$zz_yykinfo_rtncd = 1;	//リターンコード 0:正常 1:エラー
					...
					while( $zz_row = mysql_fetch_array($zz_result) ){
						$zz_yykinfo_cancel_staff_nm = $zz_row[0];	//予約取消を行ったスタッフ名
					}
				}
			}
			...
			//現在有効な予約が残っているか？
			$zz_yykinfo_new_ymd = "";
			$zz_yykinfo_new_jkn_kbn = "";
			$zz_yykinfo_new_st_time = "";
			$zz_yykinfo_new_ed_time = "";
			if( $zz_yykinfo_rtncd == 0 ){
				$zz_query = 'select YMD,JKN_KBN from D_CLASS_YYK where KG_CD = "' . $DEF_kg_cd . '" and YYK_NO = ' . $zz_yykinfo_yyk_no . ';';
				$zz_result = mysql_query($zz_query);
				if (!$zz_result) {
					$zz_yykinfo_rtncd = 1;	//リターンコード 0:正常 1:エラー
					...
					while( $zz_row = mysql_fetch_array($zz_result) ){
						$zz_yykinfo_new_ymd = $zz_row[0];		//年月日
						$zz_yykinfo_new_jkn_kbn = $zz_row[1];	//時間区分
					}
				}
				
				if( $zz_yykinfo_new_ymd != "" && $zz_yykinfo_new_jkn_kbn != "" ){
					$zz_query = 'select ST_TIME,ED_TIME from M_CLASS_JKNWR where KG_CD = "' . $DEF_kg_cd . '" and OFFICE_CD = "' . $zz_yykinfo_office_cd . '" and CLASS_CD = "' . $zz_yykinfo_class_cd . '" and JKN_KBN = "' . $zz_yykinfo_new_jkn_kbn . '" and ST_DATE <= "' . $zz_yykinfo_new_ymd . '" and ED_DATE >= "' . $zz_yykinfo_new_ymd . '" and YUKOU_FLG = 1;';
					$zz_result = mysql_query($zz_query);
					if (!$zz_result) {
						$zz_yykinfo_rtncd = 1;	//リターンコード 0:正常 1:エラー
						...
						while( $zz_row = mysql_fetch_array($zz_result) ){
							$zz_yykinfo_new_st_time = $zz_row[0];	//開始時刻
							$zz_yykinfo_new_ed_time = $zz_row[1];	//終了時刻
						}
					}
				}
			}
